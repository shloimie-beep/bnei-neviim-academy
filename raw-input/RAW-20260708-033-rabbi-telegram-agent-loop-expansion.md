# RAW-20260708-033 - Rabbi Telegram agent loop expansion

- Raw ID: `RAW-20260708-033`
- Source: `codex_chat`
- Created at: `2026-07-08T23:13:22+03:00`
- Parse status: `registered`
- Requirement IDs: `REQ-20260708-100`
- Task IDs: `TASK-20260708-017`
- Register: `tasks-pending/2026-07-08-rabbi-telegram-ticket-agent-loop.md`
...
- `REQ-20260708-100`: Expand the Rabbi Telegram bot and in-platform helper into the same OneTime-scoped sidekick contract: Rabbi communications and internal reminders go to Rabbi Telegram when the chat ID is configured; support tickets continue to route to Shloimie/super-admin with concise Telegram dings; the Rabbi bot/helper may use OneTime-scoped contacts, communications, content, safe web research, scoped Drive map/context, and safe previews, but all external sends, provider writes, payment/access changes, Drive/Vimeo/Zoom/WAPI mutations, credentials, and cross-workspace reads remain approval-gated and tested.

## Parsed Task

- `TASK-20260708-017`: Update the Rabbi Telegram runtime context, Agent Mode prompt, and readiness/test evidence so Agent Mode can smoke-test the Rabbi bot/helper loop, save PASS/FAIL/BLOCKED into Operations drop-off, and keep full live Telegram runtime blocked only by the missing `TELEGRAM_CHAT_ID_RABBI_ELIE_SCHELLER`.

## Guardrails

- Do not commit Telegram tokens, chat IDs, credentials, raw private message bodies, setup links, access codes, or contact exports.
- Do not send a live Rabbi Telegram message until the intended chat ID is configured and verified.
- Do not send email, WhatsApp/WAPI, SMS, payments, access grants, Drive/Vimeo/Zoom mutations, DNS/account changes, or public publishing from this requirement.
- Rabbi scope remains `rabbi_sheller_provider` / `one_time_mishnah_class`.
