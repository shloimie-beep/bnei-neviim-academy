...
function classifyTelegramActionRequest(input = {}) {
  ...
  if (/\b(whatsapp|wapi|wa\.me)\b/.test(value)
    && /\b(login link|parent login|portal link|access link)\b/.test(value)) {
    return {
      kind: 'typed_action',
      ...
      inputs: {
        source_text: text,
        parent_email: (text.match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i) || [])[0] || undefined,
      },
      reason: 'parent_login_whatsapp',
    };
  }

  if (/\b(whatsapp|wapi|wa\.me)\b/.test(value)
    && /\b(link|prefill|prefilled|open whatsapp)\b/.test(value)) {
    return {
      kind: 'typed_action',
  ...
  if (/\b(whatsapp|wapi|wa\.me)\b/.test(value)
    && /\b(send|schedule|broadcast|group)\b/.test(value)) {
    return {
      kind: 'typed_action',
      action_id: /\b(group|broadcast|parents|members|participants)\b/.test(value) ? 'send_whatsapp_to_group' : 'send_whatsapp_via_wapi',
      confidence: 0.8,
      dry_run: true,
      inputs: {
        body: text,
        audience: /\brabbi|sheller|mishnayos|mishnah|member|participant\b/.test(value) ? 'Rabbi Sheller participants' : 'BNA parents',
        source_text: text,
      },
      reason: 'whatsapp_send_requires_approval',
    };
  }

  if (/\b(draft|write|compose|create|make|generate)\b.{0,70}\b(whatsapp|wapi|wa\.me)\b/.test(value)
    || /\b(whatsapp|wapi|wa\.me)\b.{0,70}\b(draft|copy|message|update)\b/.test(value)) {
    return {
      kind: 'typed_action',
      action_id: /\b(newsletter|weekly update|parent update)\b/.test(value) ? 'generate_whatsapp_from_newsletter' : 'draft_whatsapp_message',
      confidence: 0.86,
      dry_run: false,
      inputs: {
        body: text,
