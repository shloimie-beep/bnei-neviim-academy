...
function classifySpreadsheet({ filePath, normalizedHeaders = [], rows = [] }) {
  ...
  const hasContactShape = normalizedHeaders.some((header) => ['email', 'email_address', 'phone', 'phone_number'].includes(header));
...
function inventoryDownloadsSpreadsheets(options = {}) {
  ...
  const inventory = files.map((filePath) => {
    ...
    const classification = classifySpreadsheet({ filePath, normalizedHeaders: parsed.normalizedHeaders, rows: parsed.rows });
    return {
      file_name: fileName,
      extension: ext,
...
function contactName(row = {}) {
  ...
  const email = normalizeEmail(field(row, ['email', 'email_address']));
...
function contactFromRow(row = {}, source = {}) {
  const email = normalizeEmail(field(row, ['email', 'email_address', 'parent_email']));
  const phone = normalizePhone(field(row, ['phone', 'phone_number', 'parent_phone', 'mobile']));
  ...
  const uiTags = [
    ...
    `one-time-import:${ONE_TIME_LAUNCH_BATCH}`,
  ];
  return {
    ...
    source: {
      ...
      source_batch: ONE_TIME_LAUNCH_BATCH,
...
function buildOneTimeContactImportPlan(options = {}) {
  ...
  for (const candidate of candidates) {
    ...
    for (const row of parsed.rows) {
      const contact = contactFromRow(row, candidate);
  ...
  return {
    ...
    batch_id: ONE_TIME_LAUNCH_BATCH,
