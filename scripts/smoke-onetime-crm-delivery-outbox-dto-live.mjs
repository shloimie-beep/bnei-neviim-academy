...
function reportPath(ext) {
  return path.join(reportDir, `${stamp}-one-time-crm-delivery-outbox-dto-live-smoke.${ext}`);
}
...
function writeReports() {
  ...
  fs.writeFileSync(mdPath, `${[
    `# One Time CRM Delivery Outbox DTO Live Smoke - ${report.started_at}`,
    '',
...
function isDeliveryOutboxTimelineItem(item = {}) {
...
function isDeliveryOutboxConversation(item = {}) {
...
async function main() {
  ...
  await step('selected contact Activity exposes delivery outbox rows only when live outbox data exists', async () => {
    ...
    for (const card of canonicalCards.slice(0, 25)) {
      ...
      const outboxTimelineCount = timelineItems.filter(isDeliveryOutboxTimelineItem).length;
      const outboxConversationCount = conversationItems.filter(isDeliveryOutboxConversation).length;
      assert(outboxConversationCount === 0, 'Delivery outbox row appeared in selected-contact Conversations');
      inspected.push({
        timeline_count: timelineItems.length,
        conversation_count: conversationItems.length,
        delivery_outbox_timeline_count: outboxTimelineCount,
        delivery_outbox_conversation_count: outboxConversationCount,
      });
      if (outboxTimelineCount > 0) {
        report.delivery_outbox_match = true;
        return {
          inspected_candidates: inspected.length,
          matched_delivery_outbox_timeline: true,
          delivery_outbox_timeline_count: outboxTimelineCount,
          delivery_outbox_conversation_count: outboxConversationCount,
    ...
    report.status = 'skipped_no_live_delivery_outbox';
    report.inspected_candidate_count = inspected.length;
    return {
      skipped: true,
      reason: 'Canonical One Time contacts were present, but no delivery_outbox DTO was returned from the inspected production sample.',
      inspected_candidates: inspected.length,
  ...
  console.log(JSON.stringify({
    ...
    delivery_outbox_match: report.delivery_outbox_match || false,
  }, null, 2));
}
