...
const outDir = path.join(repoRoot, 'ops', 'playwright-smokes', '2026-06-30-operations-ia-local');
...
const routeChecks = [
  ...
  { view: 'contacts', section: 'overview', expectedView: 'contacts', expectedSection: 'overview' },
  { view: 'communications', section: 'overview', expectedView: 'communications', expectedSection: 'overview' },
  { view: 'content', section: 'library', expectedView: 'content', expectedSection: 'library' },
  { view: 'settings', section: 'advanced', expectedView: 'settings', expectedSection: 'advanced' },
  { view: 'service_providers', section: 'overview', expectedView: 'service_providers', expectedSection: 'overview' },
];
...
async function pageState(page) {
  return page.evaluate(() => ({
    ...
    subnavModule: document.querySelector('[data-operations-subnav]')?.getAttribute('data-current-module') || '',
    filterModule: document.querySelector('[data-operations-filter-row]')?.getAttribute('data-current-module') || '',
  }));
}
...
function assertState(route, state) {
  ...
  if (state.hasModuleToolbar) throw new Error(`${route.view} rendered deprecated module toolbar`);
  if (state.helperLaunchers !== 1) throw new Error(`${route.view} rendered ${state.helperLaunchers} helper launchers`);
  if (state.topbarHelperLaunchers !== 0) throw new Error(`${route.view} rendered helper in topbar`);
  if (state.subnavModule !== state.view) throw new Error(`${route.view} subnav module mismatch: ${state.subnavModule}`);
  const filterModules = new Set(['tasks', 'communications', 'content', 'contacts']);
  if (filterModules.has(state.view) && state.filterModule !== state.view) throw new Error(`${route.view} filter module mismatch: ${state.filterModule}`);
  if (!filterModules.has(state.view) && state.filterModule) throw new Error(`${route.view} rendered unexpected filter row: ${state.filterModule}`);
}
  ...
  try {
    for (const viewport of viewports) {
        ...
        try {
          ...
          results.push({
            ...
            state: {
              ...
              helperLaunchers: state.helperLaunchers,
              topbarHelperLaunchers: state.topbarHelperLaunchers,
              hasModuleToolbar: state.hasModuleToolbar,
              subnavModule: state.subnavModule,
      ...
      try {
        ...
        await waitForOperationsShell(page);
        const taskNavButton = page.locator('.ops-sidebar-button').filter({ hasText: 'Tasks' }).first();
        if (await taskNavButton.isVisible()) {
          await taskNavButton.click();
        } else {
          await page.evaluate(() => window.switchView('tasks'));
        }
        await page.waitForFunction(() => currentView === 'tasks', null, { timeout: 10000 });
        ...
        if (taskState.workspace !== 'rabbi_sheller_provider' || taskState.view !== 'tasks' || taskState.taskProjectFilter !== 'one_time_mishnah_class') {
          throw new Error(`Sidebar task navigation lost provider state: ${JSON.stringify(taskState)}`);
        }
        await page.locator('[data-subcategory-key="pending"]').click();
        await page.waitForFunction(() => taskFocus === 'pending', null, { timeout: 10000 });
        const pendingState = await pageState(page);
        if (pendingState.subnavModule !== 'tasks' || pendingState.filterModule !== 'tasks') {
          throw new Error(`Task subnav/filter row mismatch: ${JSON.stringify(pendingState)}`);
        }
        ...
        if (backState.workspace !== 'rabbi_sheller_provider' || backState.view !== 'tasks') {
          throw new Error(`Back navigation lost provider task state: ${JSON.stringify(backState)}`);
        }
        results.push({
          viewport,
          route: { view: 'tasks', section: 'sidebar-subnav-back' },
          url: page.url().replace(baseUrl, ''),
          state: {
            workspace: backState.workspace,
            view: backState.view,
            taskFocus: backState.taskFocus,
            taskProjectFilter: backState.taskProjectFilter,
            subnavModule: backState.subnavModule,
            filterModule: backState.filterModule,
          },
          requestCount: 0,
          consoleErrors: [],
          pageErrors: [],
    ...
    const report = {
      ...
      assertions: [
        ...
        'Left sidebar remains primary modules only; module subcategories render in the top subnav.',
        'Tasks, Communications, Content, and Contacts render compact filter rows under the subnav.',
        'Operations helper renders once as a floating launcher and is absent from the topbar.',
        'Deprecated module toolbar controls are absent from the rendered shell.',
    ...
    await writeFile(path.join(outDir, 'report.md'), [
      ...
      '- Verified dashboard, tasks, communications, contacts, content, settings, One Time program, sidebar navigation, selected-module subnav/filter rows, browser back, and required viewports.',
      '',
    ...
    console.log(`PASS Operations IA local smoke: ${path.relative(repoRoot, path.join(outDir, 'report.md'))}`);
  } finally {
    await browser.close();
    await close(server);
  }
