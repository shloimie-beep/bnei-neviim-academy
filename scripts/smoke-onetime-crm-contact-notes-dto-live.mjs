...
function reportPath(ext) {
  return path.join(reportDir, `${stamp}-one-time-crm-contact-notes-dto-live-smoke.${ext}`);
}
...
function writeReports() {
  ...
  fs.writeFileSync(mdPath, `${[
    `# One Time CRM Contact Notes DTO Live Smoke - ${report.started_at}`,
    '',
...
function isContactNoteConversation(item = {}) {
  return String(item.communication_type || '').toLowerCase() === 'contact_note';
}

function isContactNoteTimelineItem(item = {}) {
  const type = String(item.type || item.communication_type || '').toLowerCase();
  const context = item.source_context && typeof item.source_context === 'object' ? item.source_context : {};
  return type === 'contact_note' || context.source_table === 'bna_contact_communications';
}
...
async function main() {
  ...
  await step('selected canonical contact DTOs include legacy contact notes when live notes exist', async () => {
    ...
    for (const card of canonicalCards.slice(0, 25)) {
      ...
      const contactNoteConversationCount = conversationItems.filter(isContactNoteConversation).length;
      const contactNoteTimelineCount = timelineItems.filter(isContactNoteTimelineItem).length;
      inspected.push({
        conversation_count: conversationItems.length,
        timeline_count: timelineItems.length,
        contact_note_conversation_count: contactNoteConversationCount,
        contact_note_timeline_count: contactNoteTimelineCount,
      });
      if (contactNoteConversationCount > 0 || contactNoteTimelineCount > 0) {
        report.contact_note_match = true;
        return {
          inspected_candidates: inspected.length,
          matched_contact_note: true,
          contact_note_conversation_count: contactNoteConversationCount,
          contact_note_timeline_count: contactNoteTimelineCount,
          no_send: conversations.no_send !== false && timeline.no_send !== false,
        };
      }
    }

    report.status = 'skipped_no_live_contact_notes';
    report.inspected_candidate_count = inspected.length;
    return {
      skipped: true,
      reason: 'Canonical One Time contacts were present, but no selected contact_note DTO was returned from the inspected production sample.',
      inspected_candidates: inspected.length,
  ...
  console.log(JSON.stringify({
    ...
    contact_note_match: report.contact_note_match || false,
  }, null, 2));
}
