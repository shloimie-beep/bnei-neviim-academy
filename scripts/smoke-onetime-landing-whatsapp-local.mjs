...
async function run() {
  ...
  for (const item of viewports) {
    ...
    await waitForText(page, 'Give your son a love for Torah');
    ...
    if (item.openMenu) {
      ...
      await assert.equal(await page.locator('[data-nav-shell]').evaluate((node) => node.classList.contains('open')), true);
    }
