...
function buildExtensionSql(fixture) {
  ...
  const taskRows = [
    ['TEST One Time next setup task - Railway target', 'Provide or verify One Time Railway project/service/database target.', 'plan', 'technology', 'Shloimie', 'Railway target values', 'Open Railway and create/verify one-time-production, one-time-web, and one-time-postgres.'],
    ['TEST One Time setup blocker - Whapi/WAPI token', 'Provide Rabbi-owned Whapi/WAPI account, phone, and token alias.', 'clarify', 'communications', 'Shloimie', 'Whapi/WAPI token and phone', 'Add token to BNA keyholder and rerun setup check.'],
    ['TEST One Time setup blocker - Stripe sandbox', 'Provide Rabbi Stripe test credential alias and $67/month test price.', 'clarify', 'finance', 'Shloimie', 'Stripe test alias/product/price', 'Create sandbox product/price or provide existing test price ID.'],
    ['TEST One Time setup blocker - Zoom protected link', 'Provide member-gated Zoom session details.', 'clarify', 'operations', 'Rabbi Elie Scheller', 'Zoom session details', 'Provide protected Zoom link or meeting metadata.'],
  ].map((row, index) => [
    sqlLiteral(row[0]),
    sqlLiteral(row[1]),
    sqlLiteral(row[2]),
