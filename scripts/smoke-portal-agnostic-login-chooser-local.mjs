...
async function smokeLoginPage({ browser, baseUrl, pageConfig, viewport }) {
  ...
  await page.getByText('Choose which portal or workspace to open.').waitFor({ state: 'visible' });
  ...
  const checks = {
    chooserVisible: await page.getByText('Choose which portal or workspace to open.').isVisible(),
...
async function run() {
  ...
  const report = {
    ...
    target: 'provider, student, and parent password login chooser rendering',
  ...
  await writeFile(path.join(outDir, 'report.md'), [
    ...
    '',
    'Scope: local static render of provider, student, and parent password login pages with mocked `chooser_required` responses. No database, credentials, external accounts, sends, billing, or production data writes.',
    '',
    ...
    'Checks:',
    '',
    '- Provider, student, and parent password login pages render server-resolved chooser destinations.',
