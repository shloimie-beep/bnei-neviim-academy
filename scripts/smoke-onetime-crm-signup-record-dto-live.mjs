...
function reportPath(ext) {
  return path.join(reportDir, `${stamp}-one-time-crm-signup-record-dto-live-smoke.${ext}`);
}
...
function writeReports() {
  ...
  fs.writeFileSync(mdPath, `${[
    `# One Time CRM Signup Record DTO Live Smoke - ${report.started_at}`,
    '',
...
function isSignupRecordTimelineItem(item = {}) {
  const type = String(item.type || item.communication_type || '').toLowerCase();
  const source = String(item.source || '').toLowerCase();
  const context = parseContext(item.source_context);
  return type === 'signup_record' ||
    source === 'signups' ||
    context.source_table === 'signups';
}

function isSignupRecordConversation(item = {}) {
...
function assertSignupRecordRedacted(item = {}) {
  const context = parseContext(item.source_context);
  assert(context.source_table === 'signups', 'signup_record source table was not signups');
  assert(context.no_send === true, 'signup_record no_send guardrail missing');
  assert(context.external_write_performed === false, 'signup_record external_write_performed guardrail missing');
  assert(context.no_checkout === true, 'signup_record no_checkout guardrail missing');
  assert(context.no_access_granted === true, 'signup_record no_access_granted guardrail missing');
  assert(context.student_name_returned === false, 'signup_record student name redaction flag missing');
  assert(context.payment_link_returned === false, 'signup_record payment link redaction flag missing');
  assert(context.checkout_session_returned === false, 'signup_record checkout session redaction flag missing');
}
...
async function main() {
  ...
  await step('selected contact Activity exposes direct signup rows only when live signup data exists', async () => {
    if (!crmCards.length) {
    ...
    for (const card of crmCards.slice(0, 25)) {
      ...
      const signupTimelineItems = timelineItems.filter(isSignupRecordTimelineItem);
      const signupConversationCount = conversationItems.filter(isSignupRecordConversation).length;
      assert(signupConversationCount === 0, 'Direct signup row appeared in selected-contact Conversations');
      inspected.push({
        timeline_count: timelineItems.length,
        conversation_count: conversationItems.length,
        signup_record_timeline_count: signupTimelineItems.length,
        signup_record_conversation_count: signupConversationCount,
      });
      if (signupTimelineItems.length > 0) {
        assertSignupRecordRedacted(signupTimelineItems[0]);
        report.signup_record_match = true;
        report.inspected_candidate_count = inspected.length;
        report.status = 'passed';
        return {
          inspected_candidates: inspected.length,
          matched_signup_record_timeline: true,
          signup_record_timeline_count: signupTimelineItems.length,
          signup_record_conversation_count: signupConversationCount,
    ...
    report.status = 'skipped_no_live_signup_records';
    report.inspected_candidate_count = inspected.length;
    return {
      skipped: true,
      inspected_candidates: inspected.length,
      reason: 'No scoped One Time direct signup_record rows were present in the inspected production contact timelines.',
    };
  });
  ...
  console.log(JSON.stringify({
    ...
    signup_record_match: report.signup_record_match || false,
  }, null, 2));
}
