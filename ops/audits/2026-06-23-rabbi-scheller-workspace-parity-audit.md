...
| Workspace user/role management and service-provider studio | MERGED | This clean branch starts from `origin/master` at `a9528b2d9467174d76d4c25bfb028f9308f24b4f`, which already includes the service-provider studio closeout. |
| Full provider-platform parity for Rabbi Scheller | PARTIAL | Provider-level shells, data models, role tests, directory, workspace-user management, and Studio are present from the clean `origin/master` base; this pass adds auth/navigation/isolation/API-usage readiness repairs. API usage backend persistence/metering remains deferred, and PR #15 is not merged. |
| Generic portal-agnostic identity/login layer | LIVE VERIFIED | Existing route-specific auth plus Operations fallback exists; this pass adds a shared server-side destination resolver for Operations/provider/parent/student password credentials, generic `/api/bna/auth/login` chooser support, wrong-portal redirect/chooser handling, operations/provider/student/parent password-login chooser rendering, and generic multi-portal logout. Pushed on branch `codex/rabbi-scheller-parity-20260624`, deployed to Railway deployment `5e37d2a0-7e81-4339-a721-c4286e8ecaa8`, and live-smoked. |
| API usage readiness foundation added in this pass | LIVE VERIFIED foundation | `src/lib/bna/provider-api-usage.js`, `tests/provider-api-usage-readiness.test.js`; includes a sanitized recorder interface future bot code can call and a feature-flagged Provider Portal empty-state preview; no DB persistence wired. Provider preview markers were live-smoked. |
| Route inventory and navigation audit artifact | PUSHED | `scripts/build-rabbi-scheller-route-map.mjs`, generated route map with 738 Express route patterns, route-map contract test, and local Provider Portal navigation smoke that walks every supported section. |
| Push/merge/deploy/live verification for this pass | PARTIAL | Shloimie chose Option A on 2026-06-24 and required a collision check first. Branch `codex/rabbi-scheller-parity-20260624` is pushed and draft PR #15 exists. Railway production deployment `5e37d2a0-7e81-4339-a721-c4286e8ecaa8` succeeded. Standard live app smoke and Rabbi workspace live smoke passed. PR #15 is not merged. |

## Git and deployment truth

| Item | Value |
|---|---|
| Current branch | `codex/rabbi-scheller-parity-20260624` |
| Local HEAD | `8f8b0b458a95d146777808dbdf1f760618632615` implementation commit |
| Tracking branch | `origin/codex/rabbi-scheller-parity-20260624` |
| Remote HEAD | `a9528b2d9467174d76d4c25bfb028f9308f24b4f` |
| `origin/master` | `a9528b2d9467174d76d4c25bfb028f9308f24b4f` |
| Ahead/behind vs `origin/master` | Feature branch is 1 commit ahead of `origin/master` before final evidence commit. |
| Current branch upstream status | pushed and tracking `origin/codex/rabbi-scheller-parity-20260624` |
| Local dirty state | final evidence updates staged for second commit; shared dirty checkout is intentionally untouched |
| Pushed from this pass | `8f8b0b458a95d146777808dbdf1f760618632615` to `origin/codex/rabbi-scheller-parity-20260624`; draft PR #15 |
| Merged from this pass | not merged |
| Deployed from this pass | Railway production deployment `5e37d2a0-7e81-4339-a721-c4286e8ecaa8` |
| Live-verified from this pass | `ops/live-smokes/2026-06-24T07-01-44-515Z-live-app-smoke.md`; `ops/live-smokes/2026-06-24T07-05-37-232Z-rabbi-scheller-workspace-live-smoke.md` |
...
|---|---|
| Express route patterns scanned | 738 |
...
| Service-provider studio and workspace users are present from the clean `origin/master` base. | MERGED / included in base | This isolated branch was created from `origin/master` after the Studio merge; no duplicate Studio implementation was added. |
| Full live Operations graph walk at 390x844, 768x1024, 1440x900 was not run. | PARTIAL | Local fixture-backed Operations Rabbi workspace smoke covers Dashboard, Tasks, Members, Program/Schedule, Communications, API Usage, Settings, toolbar navigation, browser Back/deep-link reload, scoped requests, no BNA-scoped requests, no super-admin nav, no failed requests/console errors, and no horizontal overflow at all three required viewports. Local Provider Portal navigation/API Usage and portal chooser smokes also passed. Production live smoke verifies deployed provider/API Usage/login/Operations Rabbi workspace markers and scoped task API readback, but does not replace a full authenticated live click graph at every viewport. |
...
| Staff/users/roles | `origin/master` workspace user management | Included through clean `origin/master` base | MERGED | Share existing workspace-user capability with provider-scoped permissions; no super-admin expansion. |
...
| Usage/API usage | Operations API Usage placeholder | New utility foundation; Operations honest empty state; Provider Portal feature-flagged preview | LIVE VERIFIED foundation | Feature-flag UI until real metering persistence. |
...
- Action registry JSON parse, watchdog action-registry test, and registry contracts passed for `ACTION-PROVIDER-SECTION-NAVIGATION`, `ACTION-PROVIDER-API-USAGE-PREVIEW-NAV`, and `ACTION-PORTAL-LOGIN-CHOOSER-DESTINATION`.
- Ledger JSONL parse: passed, 1,303 lines after final append.
- Run CLI validation: passed.
- Current active run pointer is `ops/execution-runs/2026-06-23-service-provider-studio`; it validates as fully done with no next unblocked batch. This Rabbi audit remains tracked in this dated register.

Not run:

- Full authenticated Operations/provider live graph walk at 390x844, 768x1024, 1440x900.
- Full live click graph at all three viewports is not run; local viewport smokes plus production marker/API live smoke are the current evidence.

Reason: local fixture-based browser evidence is available for the new Provider Portal API Usage preview and portal login chooser UI, and production live smoke verifies the deployed bundle, but the full authenticated live click graph remains a future hardening pass.

## Remaining blockers and Decisions

| ID | Decision / blocker | Owner | Status | Blocks |
|---|---|---|---|---|
| DEC-20260623-006 | Authorize push/deploy/live-smoke for any repaired Rabbi Scheller workspace bundle. | Shloimie | Decided | Use isolated branch `codex/rabbi-scheller-parity-20260624`; do not mix in the dirty shared checkout. |
...
| Branch reconciliation | Clean isolated branch starts from current `origin/master`; shared dirty checkout remains untouched. | Codex | Closed for this rollout | Avoiding collision with another agent's work. |
| Browser/live verification | Local Operations Rabbi workspace, Provider Portal navigation, Provider Portal API Usage preview, and portal chooser smokes passed; full authenticated live Operations/provider graph walk follows deploy where credentials allow it. | Codex | In progress | Final UI/navigation evidence. |
