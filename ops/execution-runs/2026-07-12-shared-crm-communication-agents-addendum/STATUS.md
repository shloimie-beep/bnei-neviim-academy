...
- Explicit CRM Create task action slice is locally verified and pending deploy: the dead-end `ACTION-CRM-CREATE-TASK-PENDING` placeholder is replaced by active `ACTION-CRM-CREATE-TASK`, the client only creates a task after an explicit click with `create_follow_up_task: true`, read-only preview disables the action, and local smoke confirms the task action is visible across split-shell and monolith at 1440/1024/768/430/390.
