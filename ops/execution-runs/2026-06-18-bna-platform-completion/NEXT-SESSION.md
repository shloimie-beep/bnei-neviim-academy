...
node --test tests/operations-workspace-selector.test.js tests/workspace-auth.test.js tests/workspace-http-isolation.test.js
```

Then inspect whether `REQ-20260618-126` has all local selector/scoped-user evidence recorded or needs an additional browser/DOM assertion. Do not close live-required workspace items as `done` without release approval, deployment, and live-smoke evidence.

Still open after this batch:

- REQ-20260618-124 remains `needs_verification` because local implementation and tests pass, but release approval, deployment, and live smoke are still pending.
