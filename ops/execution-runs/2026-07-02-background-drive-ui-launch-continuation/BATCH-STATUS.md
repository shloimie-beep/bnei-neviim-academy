...
| B5 | blocked_external_provider_inputs | Provider setup readback recorded; Railway target, DB, join-domain, Drive, and Vimeo are ready. Full setup still needs Zoom alias, Stripe sandbox/price alias, Whapi/WAPI details, and campaign approvals. |
| B6 | done | Top task view now points to GoDaddy DNS for `join.onetimeonetime.com`. |
| B7 | blocked_external_full_launch | Final validation and repo closeout are current; 2026-07-09 Railway target context can see `one-time-web` and a usable `DATABASE_URL`. Full launch bootstrap/live smoke remain blocked by the remaining external setup values, while immediate public lead capture/free-class follow-up moved to RAW-20260709-008. |
