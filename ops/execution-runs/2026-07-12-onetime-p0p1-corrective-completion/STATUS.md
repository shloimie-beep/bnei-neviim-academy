...
Run is no longer the active execution run. Release/live proof remains recorded,
and the remaining open items are carried as blockers/evidence dependencies
inside this historical run while `ops/execution-runs/latest.json` points at the
current One Time CRM/portal production-correction run.
