...
objective. Four terminal-mapping batches now cover 43 mapped or active rows:
the July 10 rows, early foundational June 16-21 rows, June 22-24 historical
rows, and June 26-July 1 rows. 249 rows still lack terminal status. Full
...
| `REQ-20260710-027` | In progress - fourth batch mapped | `historical-source-statement-matrix.json` created 291 stable `HIST-SRC-*` rows from the inventory. The July 10 rows, early foundational rows `HIST-SRC-0001` through `HIST-SRC-0014`, June 22-24 rows `HIST-SRC-0015` through `HIST-SRC-0026`, and June 26-July 1 rows `HIST-SRC-0027` through `HIST-SRC-0038` are mapped; 249 rows still lack terminal status. |
...
- PASS historical source-statement matrix parse:
  ...
  records 291 stable source rows; first through fourth mapped batches cover 43
  mapped or active rows, with 249 rows still lacking terminal status.
