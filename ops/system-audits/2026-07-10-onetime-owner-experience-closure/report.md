...
objective. Ten terminal-mapping batches now cover 137 mapped or active rows: the July 10 rows, early foundational June 16-21 rows, June 22-24 historical rows, June 26-July 1 rows, July 1-2 launch/UI rows, July 2-6 Studio/release/workflow rows, the July 6 integration/release/workflow rows, the July 7 mailbox/Agent Mode/UI/clean-launch/audit-fix rows, the July 8 parent invite/Agent Review/classroom/media/Vimeo/signup/runtime rows, and the July 8-9 performance/Telegram/helper/lag rows. 155 rows still lack terminal status. Full
...
| `REQ-20260710-027` | In progress - tenth batch mapped | `historical-source-statement-matrix.json` created 291 stable `HIST-SRC-*` rows from the inventory. The first nine batches plus July 8-9 rows `HIST-SRC-0111` through `HIST-SRC-0132` are mapped; 155 rows still lack terminal status. |
...
- PASS `npm run audit:governance`: report generated at
  `ops/audit-governance/2026-07-10T13-08-15-765Z-audit-governance.md`.
...
- PASS historical source-statement matrix parse:
  ...
  records 291 stable source rows; first through tenth mapped batches cover 137 mapped or active rows, with 155 rows still lacking terminal status.
...
1. `REQ-20260710-027`: replace the skeleton statuses in
   `ops/system-audits/2026-07-10-onetime-owner-experience-closure/historical-source-statement-matrix.json`
   with terminal statuses and evidence/blockers for the remaining 155 rows.
