...
objective. Thirteen terminal-mapping batches now cover 179 mapped or active rows: the first twelve batches plus the June 14-15 workspace/helper/classroom/payments/assistant task-register rows. 113 rows still lack terminal status. Full
...
| `REQ-20260710-027` | In progress - thirteenth batch mapped | `historical-source-statement-matrix.json` created 291 stable `HIST-SRC-*` rows from the inventory. The first thirteen batches through `HIST-SRC-0177` are mapped; 113 rows still lack terminal status. |
...
- PASS `npm run audit:governance`: report generated at
  `ops/audit-governance/2026-07-10T13-28-43-972Z-audit-governance.md`.
...
- PASS historical source-statement matrix parse:
  ...
  records 291 stable source rows; first through thirteenth mapped batches cover 179 mapped or active rows, with 113 rows still lacking terminal status.
...
1. `REQ-20260710-027`: replace the skeleton statuses in
   `ops/system-audits/2026-07-10-onetime-owner-experience-closure/historical-source-statement-matrix.json`
   with terminal statuses and evidence/blockers for the remaining 113 rows, starting at `HIST-SRC-0178`.
