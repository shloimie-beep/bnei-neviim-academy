...
objective. Six terminal-mapping batches now cover 63 mapped or active rows: the July 10 rows, early foundational June 16-21 rows, June 22-24 historical rows, June 26-July 1 rows, July 1-2 launch/UI rows, and July 2-6 Studio/release/workflow rows. 229 rows still lack terminal status. Full
...
| `REQ-20260710-027` | In progress - sixth batch mapped | `historical-source-statement-matrix.json` created 291 stable `HIST-SRC-*` rows from the inventory. The July 10 rows, early foundational rows `HIST-SRC-0001` through `HIST-SRC-0014`, June 22-24 rows `HIST-SRC-0015` through `HIST-SRC-0026`, June 26-July 1 rows `HIST-SRC-0027` through `HIST-SRC-0038`, July 1-2 rows `HIST-SRC-0039` through `HIST-SRC-0045`, and July 2-6 rows `HIST-SRC-0046` through `HIST-SRC-0058` are mapped; 229 rows still lack terminal status. |
...
- PASS historical source-statement matrix parse:
  ...
  records 291 stable source rows; first through sixth mapped batches cover 63 mapped or active rows, with 229 rows still lacking terminal status.
...
1. `REQ-20260710-027`: replace the skeleton statuses in
   `ops/system-audits/2026-07-10-onetime-owner-experience-closure/historical-source-statement-matrix.json`
   with terminal statuses and evidence/blockers for the remaining 229 rows.
