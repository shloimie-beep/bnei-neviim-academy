...
objective. Three terminal-mapping batches now cover 31 mapped or active rows:
the July 10 rows, early foundational June 16-21 rows, and June 22-24 historical
rows. 261 rows still lack terminal status. Full production launch is still not
...
| `REQ-20260710-027` | In progress - third batch mapped | `historical-source-statement-matrix.json` created 291 stable `HIST-SRC-*` rows from the inventory. The July 10 rows, early foundational rows `HIST-SRC-0001` through `HIST-SRC-0014`, and June 22-24 rows `HIST-SRC-0015` through `HIST-SRC-0026` are mapped; 261 rows still lack terminal status. |
...
- PASS historical source-statement matrix parse:
  ...
  records 291 stable source rows; first, second, and third mapped batches cover 31
  mapped or active rows, with 261 rows still lacking terminal status.
...
1. `REQ-20260710-027`: replace the skeleton statuses in
   `ops/system-audits/2026-07-10-onetime-owner-experience-closure/historical-source-statement-matrix.json`
   with terminal statuses and evidence/blockers for the remaining 261 rows.
