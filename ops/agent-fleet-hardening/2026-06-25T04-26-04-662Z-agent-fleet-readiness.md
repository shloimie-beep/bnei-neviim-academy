...
Generated: 2026-06-25T04:26:04.661Z
...
|---|---|---|
| start | `cd C:\Users\User\Documents\Codex\2026-06-24\issue-20-parent-run ; npm run agent:fleet:start` | starts hidden watcher under the current Windows login with bounded retries |
| stop | `cd C:\Users\User\Documents\Codex\2026-06-24\issue-20-parent-run ; powershell -ExecutionPolicy Bypass -File scripts/start-agent-fleet.ps1 -Stop` | stops only the PID recorded in the agent-fleet lock |
| restart | `cd C:\Users\User\Documents\Codex\2026-06-24\issue-20-parent-run ; npm run agent:fleet:restart` | stops the recorded PID, clears stale lock, and starts with bounded retries |
| status | `cd C:\Users\User\Documents\Codex\2026-06-24\issue-20-parent-run ; npm run agent:fleet:status` | reads local lock plus API queue/status when credentials are available |
| open_log | `cd C:\Users\User\Documents\Codex\2026-06-24\issue-20-parent-run ; powershell -ExecutionPolicy Bypass -File scripts/start-agent-fleet.ps1 -OpenLog` | opens local output/error logs without printing secrets into chat |
| watchdog_start | `cd C:\Users\User\Documents\Codex\2026-06-24\issue-20-parent-run ; npm run watchdog:start` | starts watchdog mode on the existing supervisor script |
| watchdog_stop | `cd C:\Users\User\Documents\Codex\2026-06-24\issue-20-parent-run ; powershell -ExecutionPolicy Bypass -File scripts/start-watchdog.ps1 -Stop` | stops only the watchdog PID recorded in its lock |
| watchdog_status | `cd C:\Users\User\Documents\Codex\2026-06-24\issue-20-parent-run ; npm run watchdog:status` | reads watchdog lock plus API runtime status when credentials are available |
