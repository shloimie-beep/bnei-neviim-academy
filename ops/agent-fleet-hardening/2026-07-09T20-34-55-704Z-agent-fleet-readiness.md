...
Generated: 2026-07-09T20:34:55.704Z
...
| watchdog_start_no_telegram | `cd C:\Users\User\BNA v2.0 ; npm run watchdog:start:no-telegram` | starts watchdog mode without sending Telegram notifications during runtime audits |
| watchdog_start_audit_only | `cd C:\Users\User\BNA v2.0 ; npm run watchdog:start:audit-only` | starts local-report-only watchdog mode with Telegram notifications, soft repairs, and runtime status writes disabled |
| watchdog_restart_no_telegram | `cd C:\Users\User\BNA v2.0 ; npm run watchdog:restart:no-telegram` | restarts watchdog mode without sending Telegram notifications during runtime audits |
| watchdog_restart_audit_only | `cd C:\Users\User\BNA v2.0 ; npm run watchdog:restart:audit-only` | restarts local-report-only watchdog mode with Telegram notifications, soft repairs, and runtime status writes disabled |
