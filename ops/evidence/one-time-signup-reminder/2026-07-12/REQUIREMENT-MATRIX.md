...
| Phone optional for email/no-reminder choices | Locally proven | `buildOneTimeSignupLeadInput()` tests; Playwright phone marker hidden before WhatsApp | Live deployed form smoke still required. |
...
| Exact three-contact local-tag preview | Locally proven at pure-function level | `buildLocalClassSegmentPreview()` expected count 3 and masked references; test covers duplicate/suppression rows | Real scoped DB preview for exactly three contacts still requires approved CRM/test DB readback and must stop on mismatch. |
