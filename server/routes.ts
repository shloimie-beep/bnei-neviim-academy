...
// Document thumbnail upload multer setup
const documentThumbnailUploadDir = path.join(process.cwd(), "uploads", "document-thumbnails");
if (!fs.existsSync(documentThumbnailUploadDir)) {
  fs.mkdirSync(documentThumbnailUploadDir, { recursive: true });
}

const documentThumbnailUpload = multer({
  storage: multer.diskStorage({
    destination: documentThumbnailUploadDir,
    filename: (req, file, cb) => {
      const uniqueSuffix = Date.now() + "-" + Math.round(Math.random() * 1e9);
      cb(null, uniqueSuffix + path.extname(file.originalname));
    },
...
export async function registerRoutes(
  ...
  app.delete("/api/admin/documents/:id", requireAdmin, async (req, res) => {
    try {
      ...
      // Delete thumbnail from cloud storage or disk
      if (doc.thumbnailPath) {
        if (doc.thumbnailPath.startsWith("/objects/")) {
          try {
            const objectFile = await objectStorageService.getObjectEntityFile(doc.thumbnailPath);
            await objectFile.delete();
            console.log(`[Document Delete] Deleted thumbnail from cloud storage: ${doc.thumbnailPath}`);
          } catch (err) {
            console.error(`[Document Delete] Failed to delete thumbnail from cloud storage:`, err);
          }
        } else if (fs.existsSync(doc.thumbnailPath)) {
          fs.unlinkSync(doc.thumbnailPath);
        }
      }
  ...
  // Admin: Upload document thumbnail
  app.post("/api/admin/documents/:id/thumbnail", requireAdmin, documentThumbnailUpload.single("thumbnail"), async (req, res) => {
    try {
      const doc = await storage.getDocument(req.params.id);
      if (!doc) {
        return res.status(404).json({ message: "Document not found" });
      ...
      if (doc.thumbnailPath) {
        if (doc.thumbnailPath.startsWith("/objects/")) {
          try {
            ...
            console.error(`[Thumbnail] Failed to delete old thumbnail:`, err);
          }
        } else if (fs.existsSync(doc.thumbnailPath)) {
          fs.unlinkSync(doc.thumbnailPath);
        }
      ...
      const updatedDoc = await storage.updateDocument(req.params.id, { thumbnailPath: objectPath });
      console.log(`[Thumbnail] Uploaded thumbnail for document ${doc.id}: ${objectPath}`);
  ...
  // Admin: Generate thumbnail from PDF first page
  app.post("/api/admin/documents/:id/generate-thumbnail", requireAdmin, async (req, res) => {
    try {
      const doc = await storage.getDocument(req.params.id);
      if (!doc) {
        return res.status(404).json({ message: "Document not found" });
      ...
      // Get PDF file stream from storage
      ...
      if (doc.filepath.startsWith("/objects/")) {
        ...
        pdfBuffer = fs.readFileSync(doc.filepath);
      } else {
        return res.status(404).json({ message: "PDF file not found" });
      }

      // Use pdftoppm to generate thumbnail from first page
      const tempPdfPath = path.join(documentThumbnailUploadDir, `temp-${Date.now()}.pdf`);
      const tempOutputPrefix = path.join(documentThumbnailUploadDir, `thumb-${Date.now()}`);
      ...
      try {
        // Use pdftoppm to convert first page to PNG (150 DPI for reasonable thumbnail size)
        await execAsync(`pdftoppm -png -f 1 -l 1 -r 150 "${tempPdfPath}" "${tempOutputPrefix}"`);
        
        // pdftoppm creates files with suffix like -1.png
        ...
        const bucket = objectStorageClient.bucket(bucketName);
        const objectFileStorage = bucket.file(objectName);
        
        await new Promise<void>((resolve, reject) => {
          const readStream = fs.createReadStream(generatedFile);
          const writeStream = objectFileStorage.createWriteStream({
            resumable: false,
        ...
        const updatedDoc = await storage.updateDocument(req.params.id, { thumbnailPath: objectPath });
        console.log(`[Thumbnail] Generated thumbnail for document ${doc.id}: ${objectPath}`);
      ...
      console.error("Generate thumbnail error:", error);
      res.status(500).json({ message: "Failed to generate thumbnail. Make sure poppler-utils is installed." });
    }
  });

  // Serve document thumbnails
