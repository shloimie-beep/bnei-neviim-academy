...
test('One Time parent promotional access invite has scoped email copy and setup links', () => {
  const subject = rabbiTemplateSubject('parent_trial_invite', { programName: 'One Time Mishnayos' });
  assert.match(subject, /access is ready/);
  assert.doesNotMatch(subject, /30-day|trial/i);
  ...
  assert.match(body, /Your promotional access is ready/);
  assert.match(body, /No payment is collected by this invite/);
...
test('admin parent promotional access invite route is launch-ready by default and smoke-labeled only when requested', () => {
  ...
  assert.match(server, /one-time-promotional-access/);
  ...
  assert.match(route, /One Time parent promotional access invite is not launch-ready/);
  ...
  assert.match(route, /trial_days: 0/);
  assert.match(route, /stripe_trial_enabled: false/);
  assert.match(route, /promotional_access: true/);
  assert.match(route, /billing_start_at: billingStartAt/);
  ...
  assert.doesNotMatch(route, /Math\.max\(1, Math\.min\(Number\(body\.trial_days/);
});
...
test('One Time parent setup page is isolated from Academy parent portal branding', () => {
  ...
  assert.match(setupPage, /One Time Parent Account Setup \/ Reset/);
  assert.match(setupPage, /Promotional parent access/);
  assert.doesNotMatch(setupPage, /30-day parent trial/i);
