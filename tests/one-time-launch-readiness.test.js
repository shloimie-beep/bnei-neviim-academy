...
test('generic Google Contacts exports require explicit One Time override and map phone value columns', () => {
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bna-onetime-phonebook-'));
  const fixturePath = path.join(fixtureDir, 'contacts-1.csv');
  fs.writeFileSync(
    fixturePath,
    [
      'First Name,Last Name,File As,E-mail 1 - Value,Phone 1 - Label,Phone 1 - Value,Notes',
      'Phone,Only,Phone Only,,Mobile,+972-50-111-2222,operator fixture',
      'Email,Andphone,Email Andphone,fixture@example.test,Mobile,+972-50-333-4444,operator fixture',
    ].join('\n')
  );
  try {
    const defaultPlan = buildOneTimeContactImportPlan({
      downloadsDir: fixtureDir,
      now: '2026-06-30T18:30:00.000Z',
      batchId: 'telegram-phonebook-2026-06-30',
    });
    assert.equal(defaultPlan.counts.contacts_after_dedupe, 0);

    const approvedPlan = buildOneTimeContactImportPlan({
      downloadsDir: fixtureDir,
      now: '2026-06-30T18:30:00.000Z',
      batchId: 'telegram-phonebook-2026-06-30',
      forceRabbiOneTime: true,
    });
    assert.equal(approvedPlan.force_rabbi_onetime, true);
    assert.equal(approvedPlan.batch_id, 'telegram-phonebook-2026-06-30');
    assert.equal(approvedPlan.source_files.length, 1);
    assert.equal(approvedPlan.counts.contacts_valid_before_dedupe, 2);
    assert.equal(approvedPlan.counts.contacts_after_dedupe, 2);
    assert.equal(approvedPlan.counts.no_send, 2);
    assert.equal(approvedPlan.contacts.some((contact) => contact.dedupe_key.startsWith('phone:')), true);
    assert.equal(approvedPlan.contacts.every((contact) => contact.tags.includes('one-time-no-send-until-approved')), true);
    assert.equal(approvedPlan.contacts.every((contact) => contact.tags.includes('one-time-import:telegram-phonebook-2026-06-30')), true);
  } finally {
    fs.rmSync(fixtureDir, { recursive: true, force: true });
  }
});
