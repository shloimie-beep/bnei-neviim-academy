...
test('Operations portal fallback preserves server-backed auth and safe returnTo behavior', () => {
  ...
  assert.match(server, /app.post\('\/api\/operations\/login'[\s\S]*collectPortalLoginDestinations\(\{ username, password \}\)/);
  assert.match(server, /app.post\('\/api\/operations\/login'[\s\S]*portal_redirect: true/);
  assert.match(server, /app.post\('\/api\/operations\/login'[\s\S]*issuePortalDestinationSession\(destination, res, \{ returnTo \}\)/);
});
...
test('provider portal API Usage is feature-flagged and honest until instrumentation is wired', () => {
  ...
  assert.match(provider, /let activeProviderSection = queryParams\.get\('section'\) \|\| 'overview'/);
