...
test('One Time billing sandbox E2E verifier proves no-trial lifecycle without external writes', async () => {
  const verifier = await import('../scripts/verify-onetime-billing-sandbox-e2e.mjs');
  const report = verifier.buildReport(new Date('2026-07-14T12:00:00.000Z'));

  assert.equal(report.status, 'passed');
  assert.equal(report.requirement_id, 'REQ-20260713-937');
  ...
  assert.equal(report.lifecycle.audit.states.trial, false);
