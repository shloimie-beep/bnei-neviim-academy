...
test('Parent contact detail can resolve and open the linked student record', () => {
  ...
  assert.match(operationsHtml, /const needsStudentRosterData = !oneTimeProgramLightPass && \(needsDashboardData \|\| needsLocalClassroomData \|\| needsCommunityData \|\| \['contacts', 'api_usage', 'calendar'\]\.includes\(activeView\) \|\| \(activeView === 'settings' && \['learning_portals', 'parent_portal', 'student_portal'\]\.includes\(activeSettingsSection\)\)\)/);
