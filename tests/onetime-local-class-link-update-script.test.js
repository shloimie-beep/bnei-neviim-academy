...
test('OneTime local-student current-link resend runner is explicitly approval gated', () => {
...
test('OneTime local-student current-link resend runner resolves only scoped local tags', () => {
...
test('OneTime local-student current-link resend runner sends individual OneTime email drafts only', () => {
  ...
  assert.match(script, /OneTimeOneTime Mishnah/);
  assert.doesNotMatch(script, /Bnei Neviim Academy/);
  assert.doesNotMatch(script, /BNA Academy/);
});

test('OneTime local-student current-link resend runner keeps secrets out of evidence', () => {
