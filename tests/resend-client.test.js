...
function svixSignature({ payload, secret, id, timestamp }) {
...
test('Resend webhook signature verification accepts valid Svix signature', () => {
  const timestamp = 1700000000;
  const id = 'msg_valid_signature';
  const secret = `whsec_${Buffer.from('bna test webhook secret').toString('base64')}`;
  const payload = JSON.stringify({
    type: 'email.received',
    data: { email_id: 'email_received_123' },
  });
  const signature = svixSignature({ payload, secret, id, timestamp });
  ...
  assert.equal(event.data.email_id, 'email_received_123');
});

test('Resend webhook signature verification rejects invalid signatures', () => {
  const timestamp = 1700000000;
  const secret = `whsec_${Buffer.from('bna test webhook secret').toString('base64')}`;
  assert.throws(
    () => verifyResendWebhookSignature({
      payload: JSON.stringify({ type: 'email.received', data: { email_id: 'email_received_123' } }),
      webhookSecret: secret,
      headers: {
