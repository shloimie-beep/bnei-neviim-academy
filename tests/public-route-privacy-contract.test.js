...
test('portal topbars expose only safe public navigation destinations', () => {
  assert.match(parentHtml, /aria-label="Safe portal navigation"[\s\S]*href="\/"[\s\S]*href="\/parents"[\s\S]*href="\/parent"[\s\S]*data-bna-assistant-open[\s\S]*href="\/student\/login"/);
  assert.match(studentHtml, /aria-label="Safe portal navigation"[\s\S]*href="\/"[\s\S]*href="\/parents"[\s\S]*href="\/student"[\s\S]*data-bna-assistant-open[\s\S]*href="\/parent\/login"/);
  assert.match(providerHtml, /aria-label="Safe portal navigation"[\s\S]*href="\/"[\s\S]*href="\/service-providers"[\s\S]*href="\/provider"[\s\S]*data-bna-assistant-open[\s\S]*href="\/providers\/join\?onboard=provider"/);
