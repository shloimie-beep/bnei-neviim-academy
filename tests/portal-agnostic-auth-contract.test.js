...
test('password login pages render server-resolved destination chooser responses', () => {
  for (const [name, html] of [
    ['operations', operationsLoginHtml],
    ['provider', providerHtml],
    ['student', studentHtml],
    ['parent', parentHtml],
  ]) {
    assert.match(html, /function show(?:Portal)?Chooser\(data\)/, `${name} login should render chooser responses`);
...
test('portal login chooser destination links are registered actions', () => {
  ...
  assert.equal(action?.route, '/operations-login, /provider, /student, /parent');
  ...
  assert.match(action?.selector_hint || '', /#chooser/);
...
test('generic BNA auth logout clears every server-backed portal session cookie', () => {
  ...
  assert.match(logout, /clearSessionCookie\(res\)/);
  assert.match(logout, /clearParentSessionCookie\(res\)/);
  assert.match(logout, /clearProviderSessionCookie\(res\)/);
  assert.match(logout, /clearStudentSessionCookie\(res\)/);
});
