...
test('one-time public target gate verifies join domain routes and instance config', async () => {
  const mod = await loadReleaseCaptain();
  const landingHtml = '<!doctype html><title>Give Your Son A Love For Torah | One Time Mishnayos</title><body><h1>Give your son a love for learning Torah.</h1><p>One Time Mishnayos</p><a>Sign Up Now</a></body>';
  const fetchCalls = [];
