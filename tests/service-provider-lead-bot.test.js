...
test('One Time provider-bot profile has explicit GHL-style sections without private runtime values', () => {
  ...
  assert.equal(profile.identity.assistant_subtitle, 'Public One Time lead information agent');
  ...
  assert.equal(profile.agent_model.build_qa_agent, false);
  const whatsappBinding = profile.channel_bindings.find((binding) => binding.channel === 'whatsapp');
  const emailBinding = profile.channel_bindings.find((binding) => binding.channel === 'email');
  assert.equal(whatsappBinding.agent_version, profile.version);
  assert.equal(emailBinding.agent_version, profile.version);
  assert.equal(whatsappBinding.knowledge_snapshot_ref, emailBinding.knowledge_snapshot_ref);
  assert.equal(whatsappBinding.reply_mode, 'capture_only');
  assert.equal(emailBinding.reply_mode, 'draft');
  assert.equal(whatsappBinding.create_task_on_inbound, false);
  assert.equal(emailBinding.create_task_on_inbound, false);
  assert.equal(whatsappBinding.formatting_policy.format, 'whatsapp');
  assert.equal(emailBinding.formatting_policy.format, 'email');
  assert.equal(emailBinding.formatting_policy.subject_required, true);
  ...
  assert.ok(schema.properties.channel_bindings);
  assert.deepEqual(schema.properties.scope.required, ['workspace_key', 'project_key', 'channels']);
