...
test('Content and One Time library data stay in the selected workspace', () => {
  ...
  assert.match(operationsHtml, /if \(currentWorkspaceIsOneTime\(workspaceKey\)\) return ONE_TIME_CONTENT_SUBTABS;/);
  assert.match(operationsHtml, /tab\.id !== 'one_time_library' \|\| currentWorkspaceIsGlobal\(workspaceKey\)/);
  assert.match(operationsHtml, /const activeContentSection = contentTabs\.some\(tab => tab\.id === contentSection\) \? contentSection : defaultContentSection\(\);/);
...
test('Operations admin, communications, integrations, and automations use selected workspace filters', () => {
  ...
  assert.match(operationsHtml, /const communicationDataFilters = activeView === 'communications' && communicationsSection === 'email'/);
