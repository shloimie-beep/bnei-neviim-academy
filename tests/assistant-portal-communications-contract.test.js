...
test('active email paths normalize sender identity and reject Office P', () => {
  ...
  assert.match(envExample, /RESEND_PROFILE=/);
  assert.match(envExample, /RESEND_RABBI_DOMAIN=onetimeonetime\.com/);
  assert.match(envExample, /RESEND_RABBI_FROM_EMAIL=info@onetimeonetime\.com/);
  assert.match(envExample, /RESEND_RABBI_FROM_NAME=OneTimeOneTime Mishnah/);
  assert.match(envExample, /RESEND_RABBI_REPLY_TO=info@onetimeonetime\.com/);
