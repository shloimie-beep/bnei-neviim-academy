...
- [ ] `RAW-20260710-003` / `REQ-20260710-026` through
  ...
  `REQ-20260710-027` is In progress - second batch mapped:
  `ops/system-audits/2026-07-10-onetime-owner-experience-closure/historical-source-statement-matrix.json`
  contains 291 stable `HIST-SRC-*` rows; the first July 10 mapping batch and
  the second early foundational mapping batch now cover 19 rows, leaving 273
  rows without terminal status. Remaining
