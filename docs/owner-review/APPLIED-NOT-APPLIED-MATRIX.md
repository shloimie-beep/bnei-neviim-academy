...
| Class/Stripe/Vimeo external readiness audit | Current full-system reality audit batch | Yes | No | No | `npm run owner-review:external-readiness`; `docs/owner-review/EXTERNAL-READINESS-AUDIT.md` | Class/Drive intake, Stripe no-charge previews, Vimeo/manual video-hosting previews | Real production class/job readback, Drive/transcription, Stripe sandbox/live API, and Vimeo API/upload proof remain approval/credential gated. |
