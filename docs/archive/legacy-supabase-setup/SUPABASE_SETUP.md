# Legacy Supabase Setup Instructions

Archived on 2026-06-10 during BNA task #209.

This was the old root `SUPABASE_SETUP.md` body from an early BNA migration
experiment. It is historical reference only. Current BNA operations use Railway
hosting plus production Postgres; do not run this SQL against live BNA unless a
future migration task explicitly audits and updates it.
...
2. `bna_tasks` - early Holy Flow task pipeline
3. `bna_payment_log` - payment audit trail
4. `cli_bridge_messages` - early Telegram-to-terminal routing

## Troubleshooting

If you get errors about tables already existing, the original note treated that
as acceptable because the SQL used `IF NOT EXISTS`.

If you get permission errors, the original note said to check that the Supabase
service role key was being used instead of the anon key.
