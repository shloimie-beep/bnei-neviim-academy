...
| Next requirement IDs to work | REQ-20260706-907 local verification complete; publish/deploy/live smoke next, then only OpenArt/model/vendor blockers remain |
...
|---|---|---|---|---|---|---|---|---|
| TASK-20260702-010 | studio-content-engine-live-readiness | Turn Studio from mock/internal scaffold into a polished content-to-slideshow/prompt workflow. | Codex | rabbi_sheller_provider / one_time_mishnah_class | RAW-20260702-010, RAW-20260706-907 | REQ-20260702-964, REQ-20260702-965, REQ-20260702-966, REQ-20260702-967, REQ-20260702-968, REQ-20260702-969, REQ-20260702-980, REQ-20260706-907 | Next: publish/deploy/live-smoke the worker handoff batch; live OpenArt/model/vendor calls remain blocked until account/model/privacy decisions. | Agent lifecycle | worker handoff local verified / publish-deploy pending |
...
| REQ-20260706-907 | `src/lib/bna/service-provider-studio-sidekick.js`, `src/lib/bna/studio-openart-mcp-adapter.js`, `server.js`, `public/operations.html`, registries, Studio tests | Add a local AI video worker handoff that packages source, storyboard, scene prompt pack, OpenArt request-plan metadata, review steps, and exact vendor blockers as a first-party Studio export. | PASS focused Studio/role/API/UI suite 41/41; PASS `npm run studio:smoke`; PASS `npm run pqc:validate`; PASS `npm run watchdog:actions`; PASS `npm run watchdog:protocol-drift`; screenshots in `ops/playwright-smokes/2026-07-06-one-time-studio-worker-handoff-local/` | pending | pending | publish/deploy/live smoke pending |
...
| REQ-20260706-907 | Local verified / publish-deploy pending | `raw-input/RAW-20260706-907-onetime-studio-worker-handoff-continuation.md`; `ops/playwright-smokes/2026-07-06-one-time-studio-worker-handoff-local/`; `ops/watchdog-audits/2026-07-06T12-34-watchdog-action-audit.md`; `ops/watchdog-audits/2026-07-06-product-quality-drift.md` | `src/lib/bna/service-provider-studio-sidekick.js`, `src/lib/bna/studio-openart-mcp-adapter.js`, `server.js`, `public/operations.html`, `ops/action-registry.json`, `ops/route-registry.json`, Studio tests | `node --check server.js`; `node --check src/lib/bna/service-provider-studio-sidekick.js`; focused Studio/role/API/UI tests 41/41 PASS; `npm run studio:smoke` PASS; PQC validation PASS; action watchdog PASS; protocol drift PASS; `git diff --check` PASS with line-ending warnings only | app-visible publish/deploy/live smoke not run yet; live OpenArt OAuth/MCP, generation/reference upload/credit actions, and true hosted image analysis remain blocked on account/model/privacy decisions |
