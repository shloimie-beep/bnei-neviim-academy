...
|---|---|---|---|---|---|
| REQ-20260708-007 | Done, pushed, deployed, live-smoked | Hub prompt cards show Start Audit, workflow state, start/result ref, timer, and call `/api/bna/agent-review/prompts/start` with CSRF. | `server.js`; `public/agent-review.html`; `ops/action-registry.json`; `ops/route-registry.json`; `tests/agent-review-hub.test.js` | PASS focused tests 66/66; PASS action watchdog; PASS live Agent Mode acceptance smoke. | None for this sub-scope. |
| REQ-20260708-008 | Done, pushed, deployed, live-smoked | Copy Agent Prompt now calls Start Audit first and prepends the in-progress AGR/draft ID, workflow state, started_at, idempotency key, and readback URL. | `public/agent-review.html`; `server.js` | PASS focused tests 66/66; PASS production prompt/drop-off readback. | None for this sub-scope. |
| REQ-20260708-009 | Done, pushed, deployed, live-smoked | Generated prompts include `## Required Workflow State`, keep drop-off open, save BLOCKED if blocked, API fallback, readback requirement, and final markers. | `src/lib/bna/agent-review-hub.js`; `public/agent-review-prompts/*` | PASS `npm run agent-review:prompts`; PASS focused tests 66/66; PASS production prompt readback includes the protocol pointer. | None for this sub-scope. |
| REQ-20260708-010 | Done, pushed, deployed, live-smoked | Drop-off supports blocked route/step, attempted action, observed failure, partial routes, partial helper responses, and evidence notes; API metadata stores these fields. | `server.js`; `public/agent-review-dropoff.html` | PASS focused tests 66/66; PASS live drop-off readback contains partial BLOCKED fields. | None for this sub-scope. |
| REQ-20260708-011 | Done, pushed, deployed, live-smoked | Save confirmation shows AGR ID, status, prompt key, idempotency key, readback URL, timestamp, and readback/repair links. | `public/agent-review-dropoff.html`; `server.js` | PASS focused tests 66/66; PASS live Agent Mode acceptance smoke. | None for this sub-scope. |
| REQ-20260708-012 | Done, pushed, deployed, live-smoked | Drop-off autosaves local browser drafts under prompt/run/idempotency scope and refuses secret-looking drafts. | `public/agent-review-dropoff.html` | PASS focused tests 66/66; inline script parse passed; PASS live drop-off readback contains autosave/readback UI. | None for this sub-scope. |
| REQ-20260708-013 | Done, pushed | Added registry/test coverage for start route/action, regenerated prompts, partial blocked save fields, and drop-off/readback contract. | `tests/agent-review-hub.test.js`; registries | PASS 66 focused tests; PASS `npm run watchdog:actions`; PASS `git diff --check`. | None for this sub-scope. |
| REQ-20260708-014 | Done, pushed | `RAW-20260708-002` and execution-run evidence preserve the failed partial audit, routes visited, blocker, and partial findings without marking Issue #24 complete. | `raw-input/RAW-20260708-002-agent-review-start-copy-dropoff-repair.md`; `ops/execution-runs/2026-06-26-agent-review-dropoff-repair/evidence/2026-07-08-one-time-brand-helper-toolbar-audit-failed-partial.md`; this register | PASS file/readback inspection. | None. |
| REQ-20260708-015 | Done, pushed, deployed, live-smoked | `docs/AGENT-REVIEW-AGENT-MODE-PROTOCOL.md` documents the reusable workflow; `src/lib/bna/agent-review-hub.js` generated prompts point to it; `tests/agent-review-hub.test.js` locks the doc, generator, and generated prompt pack together. | `raw-input/RAW-20260708-005-agent-mode-template-protocol-lock.md`; `docs/AGENT-REVIEW-AGENT-MODE-PROTOCOL.md`; `public/agent-review-prompts/*.md`; `tests/agent-review-hub.test.js` | PASS `npm run agent-review:prompts`; PASS focused tests 33/33; PASS watchdogs/hygiene; PASS production prompt readback. | None for this sub-scope. |
...
| Deploy/live production proof | PASS, superseded by final live evidence | Commit `8f3320a0` deployed successfully in Railway deployment `d383bf5f-8b73-488e-9c73-d64547862773`; production prompt/drop-off readbacks and Agent Mode acceptance smoke passed. |
...
| Production prompt readback | PASS, superseded stale deploy note | `https://bneineviimacademy.org/agent-review-prompts/one-time-brand-helper-toolbar-audit.md` returned 200 with `Generated: 2026-07-08T06:33:18.874Z` and includes `Reusable protocol/template: docs/AGENT-REVIEW-AGENT-MODE-PROTOCOL.md`, Start Audit, BLOCKED, and API fallback clauses. |
| Production prompt index readback | PASS, superseded stale deploy note | `https://bneineviimacademy.org/agent-review-prompts/index.json` returned 200 with `generated_at=2026-07-08T06:33:18.874Z`. |
| Production drop-off readback | PASS, superseded stale deploy note | Exact drop-off URL returned 200 and live HTML contains `blocked_route_or_step`, `partial_routes_visited`, `readbackPanel`, and the robust `data.result?.result_ref` fallback. |
| Live acceptance smoke | PASS | `npm run app:smoke:one-time-agent-mode-acceptance` passed after deployment and wrote `ops/live-smokes/2026-07-08T06-48-50-272Z-one-time-agent-mode-acceptance-live-smoke.md`. |
| Railway deploy gate | PASS, prior auth blocker superseded | Railway deployment `d383bf5f-8b73-488e-9c73-d64547862773` for service `skillful-motivation` reached `SUCCESS` on commit `8f3320a0`; production readbacks passed after deployment. |
