...
| Next requirement IDs to work | REQ-20260706-202 through REQ-20260706-210 |
...
|---|---|---|---|---|---|---|---|---|
| TASK-20260706-201 | onetime-studio-sidekick-openart-mcp | Build the scoped One Time Studio sidekick, OpenArt MCP boundary, and Studio-only Codex lane. | Codex | rabbi_sheller_provider / one_time_mishnah_class | RAW-20260706-002, RAW-20260706-003 | REQ-20260706-202..210 | Publish/deploy if safe; then connect OpenArt OAuth/model when Shloimie signs up. | Agent lifecycle | Blocked only on live OpenArt/model/upload and deploy/live-smoke |
...
| REQ-20260706-207 | src/lib/bna/one-time-studio-sidekick-policy.js, assistant policy, `/api/bna/studio/repair/plan` | Added mediated Studio repair lane with file/route/test allowlists and no raw shell/CLI/deploy/secrets/external writes. | `node --test tests/one-time-studio-sidekick-policy.test.js` | pending | pending | pending |
| REQ-20260706-208 | tests, watchdog reports, action/route registries | Added negative policy tests, static route/UI tests, browser smoke evidence, action registry coverage, and protocol drift report. | `npm run watchdog:actions`; `npm run watchdog:protocol-drift`; `node --check server.js` | pending | pending | pending |
...
| REQ-20260706-202 | Done locally | `one_time_ai_studio_operator` env-backed role; static role test; route guard blocks CRM/payments/helper execute/admin routes | server.js; src/lib/bna/one-time-role-model.js; tests/one-time-studio-operator-role.test.js | Passed targeted tests and `node --check server.js` | Needs commit/push/deploy/live-smoke |
...
| REQ-20260706-206 | Done locally | Studio Sidekick, OpenArt Prompt Export, Studio Repair Plan, OpenArt status card, screenshots | public/operations.html; registries; tests | Browser smoke screenshots in `ops/playwright-smokes/2026-07-06-one-time-studio-sidekick-local/` | Needs deploy/live-smoke |
| REQ-20260706-207 | Done locally | Studio-only repair lane allows Studio layout/functionality and denies website/admin/shell/deploy/secrets/contacts/payments/cross-workspace | assistant policy; sidekick policy; server route | `node --test tests/one-time-studio-sidekick-policy.test.js tests/assistant-scope-policy.test.js` | Needs deploy/live-smoke |
| REQ-20260706-208 | Done locally | Unit/static tests, browser smoke, PQC validation, action watchdog, protocol drift report | tests; ops/watchdog-audits; ops/product-quality-compiler; screenshots | 27 targeted tests passed; browser smoke passed; watchdog action findings 0; drift findings 0 | Needs deploy/live-smoke |
