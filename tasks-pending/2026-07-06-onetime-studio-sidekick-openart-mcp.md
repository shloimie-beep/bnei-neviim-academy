...
|---|---|---|---|---|---|---|---|---|
| TASK-20260706-201 | onetime-studio-sidekick-openart-mcp | Build the scoped One Time Studio sidekick, OpenArt MCP boundary, and Studio-only Codex lane. | Codex | rabbi_sheller_provider / one_time_mishnah_class | RAW-20260706-002, RAW-20260706-003 | REQ-20260706-202..210 | Review/merge PR #105, then run approved deploy/live-smoke; connect OpenArt OAuth/model when Shloimie signs up. | Agent lifecycle | Clean draft PR open; release gate dry-run passed; blocked on production deploy/live-smoke approval, live OpenArt/model/upload |
...
| REQ-20260706-208 | tests, watchdog reports, action/route registries | Added negative policy tests, static route/UI tests, browser smoke evidence, action registry coverage, and protocol drift report. | `npm run watchdog:actions`; `npm run watchdog:protocol-drift`; `node --check server.js`; release gate dry-run passed with no pushed-branch blockers | `52ba8b2b` | pushed to `origin/codex/onetime-studio-sidekick-20260706`; clean draft PR #105 | deploy/live-smoke approval not requested; production mutation not performed |
