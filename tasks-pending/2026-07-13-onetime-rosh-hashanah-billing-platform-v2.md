...
| `REQ-20260713-951` | correction map plus affected source files | Retire active trial/grace assumptions from current billing/product runtime while preserving archived legacy evidence. | focused Stripe/product tests and old-term audit | pending second-slice commit | pending | n/a |
| `REQ-20260713-952` through `REQ-20260713-956` | billing lifecycle, product policy, Stripe integration, migration, Operations readback, tests | Implement no-trial Rosh Hashanah promotional access policy, no failed-payment grace, disabled live writes, and legacy trial event ignore. | `node --test tests/stripe-billing-lifecycle.test.js tests/one-time-stripe-local-beta.test.js` pass 12/12 | pending second-slice commit | pending | live blocked |
...
| `REQ-20260713-951` | Partial local verified | active billing/product/runtime tests now expect no Stripe trial/no grace; archived legacy warm-lead row remains provenance | second backend policy slice | `node --test tests/stripe-billing-lifecycle.test.js tests/one-time-stripe-local-beta.test.js` pass 12/12 | broader old-term audit still needed before terminal Done |
| `REQ-20260713-952` | Partial local verified | provider-scoped policy/migration contracts updated without Connect/payout path | `railway-migration-2026-06-21-one-time-trial-referral-config.sql`, `src/lib/bna/one-time-product-system.js` | focused tests pass; `node --test tests/one-time-product-system.test.js` remains blocked by unrelated public landing WhatsApp baseline mismatch in `public/one-time/index.html` | full provider billing schema breadth remains pending |
