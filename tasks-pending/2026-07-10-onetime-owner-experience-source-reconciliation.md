...
- Eleven terminal mapping batches are recorded in
  `historical-source-statement-matrix.json`: the first ten batches plus
  the ten early task-register Drive/Telegram/provider
  rows now have terminal or active statuses; 145 rows still lack terminal
  status.
