...
- Fifteen terminal mapping batches are recorded in
  `historical-source-statement-matrix.json`: the first fourteen batches plus
  the June 17 queue/protocol/hebrew/Rabbi/website
  rows now have terminal or active statuses; 90 rows still lack terminal
  status.
