...
- Sixteen terminal mapping batches are recorded in
  `historical-source-statement-matrix.json`: the first fifteen batches plus
  the June 18-19 mobile/workspace/website/master-recovery/integrations
  rows now have terminal or active statuses; 85 rows still lack terminal
  status.
