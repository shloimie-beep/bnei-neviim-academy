...
- Ten terminal mapping batches are recorded in
  `historical-source-statement-matrix.json`: the first nine batches plus
  the twenty-two July 8-9 performance/Telegram/helper/lag
  rows now have terminal or active statuses; 155 rows still lack terminal
  status.
