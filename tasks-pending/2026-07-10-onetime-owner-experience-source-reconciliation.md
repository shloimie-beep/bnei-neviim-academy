...
- Twelve terminal mapping batches are recorded in
  `historical-source-statement-matrix.json`: the first eleven batches plus
  the June 10-14 task-register source/provider/helper/architecture
  rows now have terminal or active statuses; 126 rows still lack terminal
  status.
