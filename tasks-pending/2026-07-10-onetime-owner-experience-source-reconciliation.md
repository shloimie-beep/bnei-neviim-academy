...
- Twenty terminal mapping batches are recorded in
  `historical-source-statement-matrix.json`: the first seventeen batches plus
  the June 26-July 4 helper/service-provider/transcript/current-systems/product-quality/launch/UI/Studio/PR87
  rows, and the July 5-6 release/workflow/Telegram/ChatGPT/cleanup/Studio/UI/drive
  rows now have terminal or active statuses; 21 rows still lack terminal
  status, including 20 non-active rows still requiring mapping.
