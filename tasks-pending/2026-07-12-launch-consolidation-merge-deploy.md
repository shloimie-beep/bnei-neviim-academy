...
- Current run blocker: `REQ-20260712-112` release gate blocked by Stripe sandbox/campaign setup fields, Rabbi Telegram live-smoke proof, Agent Mode terminal proof, and the exact clean-tree merge/deploy/readback sequence. WAPI provider setup/auto-reply and Rabbi Telegram token/chat runtime are ready by redacted readback.
...
| DEC-20260712-203 | The local ramble/Telegram protocol branch is current work, but it includes workflow, generated registry, runtime, and test changes. | GitHub push token workflow scope and a dedicated verification pass for protocol/server/parser behavior. | Codex/operator release lane | Keep it out of the One Time app launch commit and release as a separate scoped protocol lane after workflow-scope clearance and conflict resolution. | Merge it into the same app launch commit; cherry-pick only non-workflow files. | Same-commit merge risks pushing a workflow change that GitHub rejects and mixing app launch stabilization with broad Telegram/runtime changes. Partial cherry-pick still requires resolving current master conflicts and weakens provenance. | Verify/push the protocol branch separately or provide workflow-scope credentials/approval for a dedicated protocol release. | REQ-20260712-205, REQ-20260712-208 | Active |
...
| REQ-20260712-205 | Blocked | One Time launch consolidation is in PR #130; protocol branch push rejected by GitHub workflow-scope rule and merge simulation shows conflicts. | ops/branch-audits/2026-07-12-launch-consolidation.md | `git push -u origin codex/launch-consolidation-20260712` PASS; `gh pr create --draft` PASS; `git push -u origin codex/ramble-protocol-telegram-unification-20260712` rejected for missing `workflow` scope; `git merge-tree` found action registry, package, server, and test conflicts. | Protocol branch needs workflow-scope token plus conflict-resolution/verification, or a dedicated no-workflow split branch after explicit scope decision. |
...
| REQ-20260712-207 | Done | Focused One Time tests and smokes passed; readiness gate blocks on true external/proof blockers. WAPI provider setup/auto-reply and Rabbi Telegram token/chat runtime now read back ready. | ops/evidence/one-time-crm-journey/2026-07-12/report.md, ops/ui-audits/2026-07-11-onetime-p0p1-corrective/public-onboarding-smoke.md, ops/production-readiness/latest-production-readiness-snapshot.md, ops/watchdog-audits/2026-07-09-onetime-wapi-readiness.md, ops/watchdog-audits/2026-07-08-rabbi-telegram-ticket-readiness.md | `npm run test:onetime:focused`; `npm run one-time:smoke:operations-crm-workbench-local`; `npm run one-time:smoke:public-onboarding-local`; `npm run watchdog:actions`; `node --test tests/one-time-external-setup-readiness.test.js`; `node --test tests/one-time-wapi-scope-contract.test.js`; `npm run production:readiness:gate -- --json`. | Gate remains blocked by Stripe sandbox setup, campaign proof, Rabbi Telegram live-smoke proof, Agent Mode proof, and merge/deploy/readback requirements. |
| REQ-20260712-208 | Blocked | Draft PR #130 and production readiness gate report. | ops/production-readiness/latest-production-readiness-snapshot.md, ops/production-readiness/latest-production-unblocker.md | `gh pr view 130` reports draft/open/mergeable; `npm run production:readiness:gate -- --json` returned blocked. | Cannot honestly merge to master/deploy/Done until Stripe/campaign setup fields, Rabbi Telegram live-smoke proof, Agent Mode proofs, and approved release/deploy readbacks are cleared. |
