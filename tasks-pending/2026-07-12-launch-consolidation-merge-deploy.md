...
| REQ-20260712-202 | Git refs/run files | Inventory branches, worktrees, PRs, active run status, and dirty work. | `git fetch`, `git branch`, `gh pr list`, `npm run bna:run:status`, `npm run bna:run:next`. | pending | pending | not required |
...
| REQ-20260712-202 | In progress | Initial branch/PR/run inventory in this register. | none yet beyond register | `git fetch --all --prune`; `gh pr list --state open`; `npm run bna:run:status`; `npm run bna:run:next`. | Need worktree/remote-branch classification and production readiness gate. |
