...
| REQ-20260712-203 | Dirty local files | Attribute dirty files to One Time release, Telegram sidekick packet, generated evidence, or unrelated lanes. | `git status`, diffs, tests by scope. | pending | pending | not required |
| REQ-20260712-204 | `master` | Safely reconcile local master with `origin/master`. | Conflict-free sync or resolved conflicts plus tests. | pending | pending | not required |
| REQ-20260712-205 | Merge candidates | Merge one branch at a time after diff review. | Focused tests per branch. | pending | pending | required if app/server visible |
| REQ-20260712-206 | Remote branch backlog | Classify and record outcome. | Branch ancestry/content audit. | pending | pending | not required unless code merged |
| REQ-20260712-207 | Test/watchdog gates | Run required verification. | npm scripts and reports. | pending | pending | not required |
| REQ-20260712-208 | Deploy/live services | Push, deploy, read back exact SHA, smoke affected live routes. | Release gate and live smoke reports. | pending | pending | required |
...
| REQ-20260712-203 | In progress | Dirty launch work staged on `codex/launch-consolidation-20260712`. | staged launch candidate | `npm run secrets:audit`; `git diff --cached --check`. | Needs clean commit after verification snapshot. |
| REQ-20260712-207 | In progress | Focused One Time tests and smokes passed; readiness gate blocks on external setup and dirty tree. | ops/evidence/one-time-crm-journey/2026-07-12/report.md, ops/ui-audits/2026-07-11-onetime-p0p1-corrective/public-onboarding-smoke.md, ops/production-readiness/latest-production-readiness-snapshot.md | `npm run test:onetime:focused`; `npm run one-time:smoke:operations-crm-workbench-local`; `npm run one-time:smoke:public-onboarding-local`; `npm run watchdog:actions`; `npm run bna:run:status`; `npm run production:readiness:gate -- --json`. | Production gate still blocked by external setup, Telegram runtime, Agent Mode proof, and clean-tree requirement. |
| REQ-20260712-208 | Blocked | Production readiness gate report. | ops/production-readiness/latest-production-readiness-snapshot.md, ops/production-readiness/latest-production-unblocker.md | `npm run production:readiness:gate -- --json` returned blocked. | Cannot honestly deploy/Done until external setup fields, Rabbi Telegram runtime, and Agent Mode proofs are cleared; dirty-tree blocker should clear after commit. |
