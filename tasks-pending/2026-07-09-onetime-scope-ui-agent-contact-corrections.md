...
| Next requirement IDs to work | `REQ-20260709-001`, `REQ-20260709-002`, `REQ-20260709-005` |

## Parsed requirements

| ID | Requirement | Source IDs | Workspace/project | Owner | Category | Priority | Batch | Dependencies | Acceptance criteria | Implementation files | Deploy/live required | Status |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `REQ-20260709-001` | Browser QA / Agent Mode blocked runs must submit and seal autonomously with evidence, not ask the operator whether to finish the run. | `RAW-20260709-001` | BNA Operations / Agent Review | Codex | agent_loop | P0 | `agent-loop-autonomy` | none | Browser QA prompt/template explicitly allows progress/evidence/result/seal writes; blocked/fail criteria tell the agent to submit/seal; tests prove blocked route/UI runs include submit/seal instructions and "do not ask operator to submit/seal". | Agent prompt generator, Agent Review prompt files/tests | yes, if server-visible prompt output changes | Local verified - commit/push/deploy pending |
| `REQ-20260709-002` | OneTime public/helper/portal routes must not show BNA Academy branding, links, bot identity, or default Academy auth reset screens. | `RAW-20260709-001` | `rabbi_sheller_provider` / `one_time_mishnah_class` | Codex | workspace_scope | P0 | `scope-bleed-guard` | route/auth inspection | Live/readback and tests show OneTime routes use OneTime brand/domain/sender/helper identity and no BNA Academy login/reset/bot copy except internal super-admin backend surfaces. | Routing, auth reset, brand config, helper config, scope tests | yes | Local verified - commit/push/deploy pending |
| `REQ-20260709-003` | Parent/provider password reset should be usable for the operator's real flow and should not expire before normal completion; no classroom fallback/recovery code UX. | `RAW-20260709-001` | `rabbi_sheller_provider` / `one_time_mishnah_class` | Codex | auth | P0 | `auth-reset` | auth/token policy inspection | OneTime reset links have a reasonable TTL or one-time reissue flow; expired reset screen is branded OneTime and offers forgot-password resend; no separate classroom/recovery code is shown by default. | Auth reset scripts/routes/templates/tests | yes | Partially local verified - student access-code fallback removed; reset TTL still open |
...
| `REQ-20260709-007` | Student view desktop/mobile layout must be visually audited and repaired for centering, spacing, card alignment, padding, and responsive controls. | `RAW-20260709-001` | `rabbi_sheller_provider` / `one_time_mishnah_class` | Codex | product_quality_ui | P0 | `visual-audit-first` | PQC packet and screenshots | Playwright screenshots prove desktop and mobile student views are centered, evenly spaced, no overflow, no overlapping controls, and professional card alignment. | Student portal/classroom components/tests | yes | In progress - OneTime student login repaired locally; full student portal audit still open |
...
|---|---|---|---|---|---|---|
| `REQ-20260709-001` | Agent prompt generator, Agent Review prompt outputs/tests | Patched deterministic Browser QA blocked-run closeout instructions and regenerated 18 Agent Mode prompt files. | `PASS npm run agent-review:prompts`; `PASS npm test` 1674/1674; `PASS npm run secrets:audit`; `PASS npm run watchdog:protocol-drift`; `PASS git diff --check` warnings only | Pending | Pending | Pending |
| `REQ-20260709-002` | OneTime brand/auth/helper routes/config/tests | Patched parent/student login redirects, OneTime provider shell intercept, student helper scoping, and OneTime student login visual/readability/access-code removal. | `PASS npm test` 1674/1674; `PASS` local desktop/mobile Playwright smoke in `ops/ui-audits/2026-07-09-onetime-scope-bleed-local/smoke-results.json`; action coverage/parity artifacts regenerated and passed | Pending | Pending | Pending |
| `REQ-20260709-005` | Provider/student UI routes/components | Fixed provider initial shell OneTime scope bleed and captured desktop/mobile smoke screenshots; full provider CRM/content shell IA remains a separate open audit. | `PASS` provider review CRM shell desktop/mobile smoke screenshots in `ops/ui-audits/2026-07-09-onetime-scope-bleed-local/` | Pending | Pending | Pending |

## Final audit

| ID | Status | Evidence | Files changed | Verification | Remaining issue |
|---|---|---|---|---|---|
| `REQ-20260709-001` | Local verified - commit/push/deploy pending | Updated `BROWSER_QA_TEMPLATE` and generated Agent Review prompt files require blocked/failed verifier runs to submit result, seal run, and not ask the operator whether to close out. | `src/lib/bna/agent-control.js`, `src/lib/bna/agent-review-hub.js`, `public/agent-review-prompts/*.md`, tests/docs/memory | `PASS npm run agent-review:prompts`; `PASS npm test` 1674/1674; `PASS secrets:audit`; `PASS watchdog:protocol-drift`; `PASS git diff --check` warnings only | Needs commit/push and deploy/live readback before Done |
| `REQ-20260709-002` | Local verified - commit/push/deploy pending | Desktop/mobile smoke passed for `/parent/login`, `/student/login`, and `/provider.html?review=one-time&section=crm`; no visible BNA Academy, no HE toggle, no student access-code fallback, OneTime student helper present. Evidence: `ops/ui-audits/2026-07-09-onetime-scope-bleed-local/smoke-results.json` and screenshots. | `server.js`, `public/student.html`, `public/js/bna-bot-widget.js`, `public/css/one-time-shared-review.css`, tests, action coverage/parity artifacts | `PASS npm test` 1674/1674; `PASS` desktop/mobile Playwright smoke | Needs commit/push and deploy/live readback before Done |
| `REQ-20260709-003` | Partial | OneTime student login no longer shows access-code fallback in the real single-tenant login route. | `public/student.html`, tests/screenshots | `PASS` desktop/mobile Playwright smoke confirms no access fallback on `/student/login` | Reset-link TTL/expired OneTime resend flow still open |
| `REQ-20260709-005` | In progress | Provider review CRM initial shell no longer shows BNA Academy in the local single-tenant/read-only review smoke. | `server.js`, screenshot `provider-review-crm*.png` | `PASS` desktop/mobile Playwright smoke | Full Rabbi provider IA/layout/CRM polish still open |
| `REQ-20260709-007` | In progress | OneTime student login desktop/mobile now readable, scoped, no BNA helper label, no HE toggle, no access-code fallback. | `public/student.html`, `public/css/one-time-shared-review.css`, screenshots `student-login*.png` | `PASS` desktop/mobile Playwright smoke | Full logged-in student portal/classroom layout audit still open |
