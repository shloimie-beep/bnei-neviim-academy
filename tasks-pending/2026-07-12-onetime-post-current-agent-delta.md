...
| Vertical CRM implementation packets | Previous CRM implementation covers DTO/list/workbench/inbox context; packet asks for OT-CRM-01 through OT-CRM-05 ownership and canonical-route mutation proof. | Blocked after safe C3/C8 slice. Local-only CRM endpoints, split-shell client methods, route/action registry rows, and canonical `/operations` browser proof are implemented; isolated DB mutation/reload proof needs `BNA_ONETIME_CRM_TEST_DATABASE_URL`. | REQ-20260712-806 |
...
| REQ-20260712-806 | Split CRM Contacts/Inbox work into OT-CRM-01 through OT-CRM-05 current-state packets and implement only missing/partial rows in dependency order. | REQ-20260711-005 / REQ-20260712-107 | Codex | crm_implementation | P0 | delta-C1-C9 | REQ-20260712-805 | yes | Blocked |
...
| REQ-20260712-805 | Done | Canonical blueprint and surface map now define current-state CRM Contacts/Inbox ownership, action state matrix, and gap matrix. | `ops/product-specs/one-time/crm/contacts-inbox.v1.json`, `ops/surface-maps/2026-07-12-one-time-crm-contacts-inbox-surface-map.md`, `ops/surface-maps/2026-07-12-one-time-crm-contacts-inbox-surface-map.json`, PQC validation report. | PASS JSON parse for spec/map; PASS focused CRM/inbox tests 20/20; PASS `npm run pqc:validate`; PASS `npm run bna:run:validate`. | `REQ-20260712-806` must implement or block only the remaining partial/open OT-CRM rows and capture canonical-route proof. |
| REQ-20260712-806 | Blocked | Safe implementation slice added local-only CRM create/detail/note/task/task-update/thread/draft endpoints, split-shell client methods, route/action registry rows, and canonical `/operations` browser proof. | `server.js`, `public/js/operations-shell.js`, `tests/service-provider-scope-routes.test.js`, `ops/action-registry.json`, `ops/route-registry.json`, `scripts/smoke-onetime-operations-crm-workbench-local.mjs`, `ops/ui-audits/2026-07-10-onetime-crm-workbench-local/report.md`. | PASS `node --check server.js`; PASS `node --check public/js/operations-shell.js`; PASS focused route/tenant/model tests 18/18; PASS canonical local browser smoke; PASS `npm run watchdog:actions`; PASS `npm run bna:run:validate`. | Blocked until `BNA_ONETIME_CRM_TEST_DATABASE_URL` points at a local/test Postgres URL and `node scripts/smoke-onetime-crm-journey-local-db.mjs` passes. |
