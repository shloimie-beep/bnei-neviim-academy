...
| Next requirement IDs to work | Continue `REQ-20260708-095`; first runtime alias batch and second read-only wrapper batch are deployed/live-smoked; parent/student summary, preview-action, and draft-only sidekick wrapper batches are local verified; full autonomy remains blocked |
...
| `TASK-20260708-016` | rabbi_helper_wrapper_batches | Implement scoped helper wrapper batches from the generated map. | Codex | `rabbi_sheller_provider` / `one_time_mishnah_class` | `RAW-20260708-028` | `REQ-20260708-095` | Continue from 66 wrapper-backed local contracts; no read-only wrappers remain missing, and the draft-only sidekick batch is now local verified, so the next safe batch should target broader internal writes with strict result-card/audit logging. | internal | Open / partial deployed + local verified |
...
| `REQ-20260708-095` | `src/lib/bna/helper/tool-registry.js`, `planner.js`, `permissions.js`, `safety.js`, parity/scope maps, helper tests, action coverage, lazy-subnav tests | First runtime alias batch plus read-only batch plus parent/student summary batch plus dry-run preview wrappers plus draft-only sidekick wrappers for calendar/email/campaign/automation/newsletter/social/worksheet/admin-message drafts. Scope-map generator treats write-shaped missing wrappers as write/draft/approval-gated instead of read-only. | PASS `node --check src/lib/bna/helper/tool-registry.js`; PASS `node --check src/lib/bna/helper/planner.js`; PASS `node --check scripts/generate-rabbi-helper-tool-scope-map.mjs`; PASS `node scripts/generate-rabbi-helper-tool-scope-map.mjs`; PASS `node scripts/generate-agent-review-prompts.cjs`; PASS `node --test tests/bna-helper-tools.test.js tests/rabbi-helper-tool-scope-map.test.js` 25/25; PASS `node --test tests/agent-review-hub.test.js` 11/11; refreshed Rabbi scope map 163 contracts with 66 `tool_wrapper_available_local`, 12 fallback blockers, and 85 missing wrappers | `9618a4d4`, `16a69c9e`, `9e611cbd`, `304d68a8`, `d23c9ec2`, `17ff491b`, `b49a74e2`; current sidekick batch commit pending | `b49a74e2`; current sidekick batch push pending | Previous Railway deployment `2107fae5-1a73-49ec-96e8-5a3a66bb8e43` `SUCCESS`; parent/student, preview, draft-only, and Rabbi Telegram sidekick batches deploy/live smoke pending a clean release path |
...
| `REQ-20260708-091` | Done / local verified | 163-contract JSON/MD map with stricter write/draft/approval policies | `scripts/generate-rabbi-helper-tool-scope-map.mjs`, `ops/helper-tool-scope/rabbi-one-time-tool-scope-map.json`, `ops/helper-tool-scope/rabbi-one-time-tool-scope-map.md` | PASS generator and helper/scope focused tests after current draft-only sidekick batch; full `npm test` pending after latest bot/context updates | Full runtime wrappers still missing for 85 contracts; 12 fallback blockers remain |
...
| `REQ-20260708-095` | Open / partial deployed + local verified / autonomy blocked | 66 contracts now `tool_wrapper_available_local`; 12 are registered fallback blockers; 85 remain `tool_wrapper_missing`; no read-only wrappers remain missing | `src/lib/bna/helper/tool-registry.js`, `src/lib/bna/helper/planner.js`, `src/lib/bna/helper/permissions.js`, `src/lib/bna/helper/safety.js`, `scripts/generate-rabbi-helper-tool-scope-map.mjs`, action coverage artifacts, scope maps, tests | PASS helper/scope focused tests 25/25; PASS Agent Review prompt tests 11/11; previous PASS push/deploy/live smoke through `d23c9ec2` for 27 contracts; latest draft-only sidekick batch full `npm test` not rerun | Full autonomy still blocked by 85 missing wrappers, 12 fallback blockers, local sidekick batches needing deploy/live smoke, and saved Agent Mode proof |
...
2. The Rabbi audit baseline still tracks 163 contracts: 66 now have local
   wrappers, 12 are registered fallback/setup blockers, and 85 remain
...
5. Of the 66 wrapper-backed contracts, the first 27 are deployed through
   Railway deployment `2107fae5-1a73-49ec-96e8-5a3a66bb8e43`; the 9
   parent/student summary wrappers, 13 dry-run preview contracts, and 17
   draft-only sidekick contracts are local-verified and still need a clean
   deploy/live smoke path. All 66 still need saved Agent Mode PASS/BLOCKED
   proof before autonomy can be claimed.
...
11. Agent Mode has not yet saved a PASS/BLOCKED/FAIL drop-off result for the
   all-163 probe run.
12. Latest draft-only sidekick batch verification is focused: helper/scope
   tests passed 25/25 and Agent Review prompt tests passed 11/11. Full
   `npm test` was not rerun after this batch; the last recorded full-suite pass
   was 1664/1664 before the current draft-only wrapper additions.
