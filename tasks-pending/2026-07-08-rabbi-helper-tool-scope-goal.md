...
| Next requirement IDs to work | Continue `REQ-20260708-095`; all 163 contracts and account-bot scope provisioning are local verified, pushed, and deployed/live-smoked on current OneTime Railway deployment `cc221c00-1cb3-4b66-b995-40ad78453096`; full autonomy remains blocked by saved all-163 Agent Mode proof, external approval gates, and per-tool live audit readback |
...
| `REQ-20260708-095` | Implement the scoped helper wrappers, planner intents, result cards, audit writes, and live Agent Mode proof for the mapped contracts. | `RAW-20260708-028` | `rabbi_sheller_provider` / `one_time_mishnah_class` | Codex | implementation | P0 | 2+ | `REQ-20260708-091` through `REQ-20260708-094` | Each contract moves from `tool_wrapper_missing` or fallback-only placeholder to executable only after server-side scope filters, planner coverage, permission gates, destination/result-card scope, redacted audit logging, negative tests, Agent Mode saved proof exist, and account-bot subaccount scopes can be materialized from the template into server-enforced allow/deny lists. | `src/lib/bna/helper/tool-registry.js`, `src/lib/bna/helper/planner.js`, `src/lib/bna/helper/permissions.js`, `src/lib/bna/helper/account-scope-provisioning.js`, `src/lib/bna/helper/safety.js`, `src/lib/bna/helper/destination-resolver.js`, `server.js`, follow-up tests | yes for app/server-visible behavior | Open / 163 local and deployed; account-scope provisioning local verified / full autonomy blocked by Agent Mode proof, external gates, and live audit readback |
...
| `REQ-20260708-095` | `src/lib/bna/helper/tool-registry.js`, `planner.js`, `permissions.js`, `account-scope-provisioning.js`, `safety.js`, parity/scope maps, helper tests, action coverage, lazy-subnav tests | First runtime alias batch plus read-only batch plus parent/student summary batch plus dry-run preview wrappers, draft-only sidekick wrappers, internal-action wrappers, content/provider action wrappers, scoped packet wrappers, final approval/provider packet wrappers, final fallback-replacement scoped packet wrappers, and account-bot subaccount scope provisioning that materializes template examples into server-enforced helper allow/deny lists. Scope-map generator treats write-shaped wrappers as write/draft/approval-gated and preserves deliberate read-only readiness wrappers. | PASS `node --check src/lib/bna/helper/tool-registry.js`; PASS `node --check src/lib/bna/helper/planner.js`; PASS `node --check src/lib/bna/helper/account-scope-provisioning.js`; PASS `node --check src/lib/bna/helper/permissions.js`; PASS `node --check scripts/generate-rabbi-helper-tool-scope-map.mjs`; PASS `node scripts/generate-rabbi-helper-tool-scope-map.mjs`; PASS focused helper/scope/Agent Review/OneTime reset tests; PASS full `npm test` 1687/1687; PASS `npm run secrets:audit`; PASS `npm run watchdog:protocol-drift`; refreshed Rabbi scope map 163 contracts with 163 `tool_wrapper_available_local`, 0 fallback blockers, and 0 missing wrappers | through `6a570345`; public prompt support through `f9eb486d` / `f907a87e` | through `f907a87e` | OneTime Railway deployment `cc221c00-1cb3-4b66-b995-40ad78453096` reached `SUCCESS`; `npm run app:smoke:onetime-separate-instance -- https://join.onetimeonetime.com` passed; `npm run app:smoke:rabbi-onetime-landing -- https://join.onetimeonetime.com` passed; live readback of `/agent-review-prompts/rabbi-helper-tool-scope-map.md` and all 3 public artifacts returned `200` with expected scope markers. |
...
| `REQ-20260708-092` | Done / local verified | Template JSON plus provisioner coverage from `REQ-20260708-095` | `ops/helper-tool-scope/account-bot-scope-template.json`, `src/lib/bna/helper/account-scope-provisioning.js`, `tests/rabbi-helper-tool-scope-map.test.js` | PASS focused scope tests 18/18 | None |
...
| `REQ-20260708-095` | Open / 163 local wrapper-backed contracts deployed/live-smoked / account-scope provisioning deployed / autonomy blocked | 163 contracts now `tool_wrapper_available_local`; 0 are registered fallback blockers; 0 remain `tool_wrapper_missing`; no read-only wrappers remain missing; account-bot provisioning now materializes the Benny tasks/studio template into allowed and forbidden helper tool IDs and runtime permissions enforce that narrowed scope before role shortcuts. | `src/lib/bna/helper/tool-registry.js`, `src/lib/bna/helper/planner.js`, `src/lib/bna/helper/permissions.js`, `src/lib/bna/helper/account-scope-provisioning.js`, `src/lib/bna/helper/safety.js`, `scripts/generate-rabbi-helper-tool-scope-map.mjs`, action coverage artifacts, scope maps, tests | PASS focused helper/scope/Agent Review/OneTime reset/RBAC tests; PASS full `npm test` 1687/1687; PASS `npm run secrets:audit`; PASS `npm run watchdog:protocol-drift`; PASS Railway deployment `cc221c00-1cb3-4b66-b995-40ad78453096` `SUCCESS`; PASS OneTime separate-instance live smoke; PASS Rabbi landing live smoke; PASS live prompt/artifact readbacks with 163-scope markers | Full autonomy still blocked by saved all-163 Agent Mode PASS/FAIL/BLOCKED drop-off proof, `DEC-20260708-019` external/write approvals, and per-tool live audit readback evidence |
...
## Local Verification - Account Bot Scope Provisioning Batch

- Added `src/lib/bna/helper/account-scope-provisioning.js` to materialize
  `ops/helper-tool-scope/account-bot-scope-template.json` into concrete
  `allowed_tool_ids` and `forbidden_tool_ids` for subaccounts such as
  `benny_studio_tasks_bot`.
- Runtime helper permissions now apply the provisioned account scope before
  broad admin/provider role shortcuts, so a narrowed bot account cannot use
  helper tools outside its generated allow list.
- The provisioner backfills missing workspace/project/provider scope from the
  canonical Rabbi scope map and preserves server-side invariants:
  server recomputes scope, client scope is untrusted, and cross-workspace
  access is false.
- Guardrail: the Benny tasks/studio example allows tasks/studio tools such as
  `create_rabbi_source_sheet_task`, `create_one_time_video_library_item`,
  `generate_worksheet`, and `transcribe_video`; it forbids contact/CRM,
  provider setup, agent-ops, communications, and approval-gated external-write
  tools such as `create_contact`, `create_provider_lead`,
  `record_agent_result`, `approve_email`, and `sync_google_calendar`.
- Verification passed:
  `node --check src/lib/bna/helper/account-scope-provisioning.js`;
  `node --check src/lib/bna/helper/permissions.js`;
  `node --test tests/rabbi-helper-tool-scope-map.test.js` passed 18/18;
  `node --test tests/workspace-rbac-negative-isolation.test.js tests/one-time-rbac-negative-isolation.test.js`
  passed 6/6; `node --test tests/bna-helper-tools.test.js` passed 14/14;
  full `npm test` passed 1687/1687.
- Next action: run closeout checks, push the scoped commit, deploy through the
  guarded OneTime Railway path, and live-smoke the deployed app.

## Deploy And Live Smoke - Account Scope Provisioning

- Commit `6a570345` (`Enforce helper account scope provisioning`) was pushed
  to `origin/codex/rabbi-helper-tool-scope-20260708`.
- OneTime Railway deployment
  `36874255-af07-47a8-a41a-0f8919af801b` reached `SUCCESS` for the
  account-scope provisioning batch.
- The later Agent Review public prompt deployment
  `cc221c00-1cb3-4b66-b995-40ad78453096` also includes the account-scope
  provisioning code and reached `SUCCESS`.
- Live smokes passed after deployment:
  `npm run app:smoke:onetime-separate-instance -- https://join.onetimeonetime.com`,
  `npm run app:smoke:rabbi-onetime-landing -- https://join.onetimeonetime.com`,
  and `npm run one-time:target:guard -- --json`.
