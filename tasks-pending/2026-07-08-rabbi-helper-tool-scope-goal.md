...
| Next requirement IDs to work | Continue `REQ-20260708-095`; first runtime alias batch and second read-only wrapper batch are deployed/live-smoked, the parent/student summary wrapper batch is local verified, and full autonomy remains blocked |
...
| `TASK-20260708-016` | rabbi_helper_wrapper_batches | Implement scoped helper wrapper batches from the generated map. | Codex | `rabbi_sheller_provider` / `one_time_mishnah_class` | `RAW-20260708-028` | `REQ-20260708-095` | Continue from 36 wrapper-backed local contracts; next safe batch should target the remaining 30 read-only missing wrappers before broader internal writes. | internal | Open / partial deployed + local verified |
...
|---|---|---|---|---|---|---|
| `REQ-20260708-091` | `ops/helper-tool-scope/*`, generator script, `ops/helper-tool-parity-map.*` | Preserve the 163-contract audit baseline while covering all 127 current `tool_needed` parity rows after the parent/student summary wrapper batch. | PASS `node --check scripts/generate-rabbi-helper-tool-scope-map.mjs`; PASS `node scripts/generate-rabbi-helper-tool-scope-map.mjs`; PASS `node scripts/generate-helper-tool-parity-map.mjs`; PASS focused tests 28/28 | Pending | Pending | Not required |
...
| `REQ-20260708-095` | `src/lib/bna/helper/tool-registry.js`, `planner.js`, `permissions.js`, `safety.js`, parity/scope maps, helper tests | First runtime alias batch plus read-only batch plus parent/student summary batch: `list_students`, `show_assignments`, `show_child_calendar`, `view_parent_visible_notes`, `show_my_assignments`, `show_my_goals`, `show_parent_students`, `show_student_progress`, `show_student_progress_for_parent`. | PASS focused helper/scope/RBAC tests 28/28; PASS syntax checks; refreshed parity map 270 rows and Rabbi scope map 163 contracts with 36 `tool_wrapper_available_local`; BROAD BLOCKED `npm test` 1658/1662 due unrelated Operations shell/action-coverage drift in `public/operations.html` and `ops/action-registry/one-time-action-coverage.*`; PASS live prompt readback from prior deploy | `9618a4d4`, `16a69c9e`, `9e611cbd`, `304d68a8`, `d23c9ec2`; parent/student summary batch commit pending | `d23c9ec2`; parent/student summary batch push pending | Previous Railway deployment `2107fae5-1a73-49ec-96e8-5a3a66bb8e43` `SUCCESS`; parent/student summary batch deploy/live smoke pending a clean release path |
...
| `REQ-20260708-095` | Open / partial deployed + local verified / autonomy blocked | 36 contracts now `tool_wrapper_available_local`; 12 are registered fallback blockers; 115 remain `tool_wrapper_missing` | `src/lib/bna/helper/tool-registry.js`, `src/lib/bna/helper/planner.js`, `src/lib/bna/helper/permissions.js`, `src/lib/bna/helper/safety.js`, `scripts/generate-rabbi-helper-tool-scope-map.mjs`, parity/scope maps, tests | PASS `node --test tests/bna-helper-tools.test.js tests/rabbi-helper-tool-scope-map.test.js tests/one-time-rbac-negative-isolation.test.js tests/workspace-rbac-negative-isolation.test.js` (28/28); PASS syntax checks; BROAD BLOCKED `npm test` 1658/1662 due unrelated Operations shell/action-coverage drift; previous PASS push/deploy/live smoke through `d23c9ec2` for 27 contracts | Full autonomy still blocked by 115 missing wrappers, 12 fallback blockers, parent/student summary batch deploy/live smoke, and saved Agent Mode proof |
...
1. The refreshed base helper parity map has 127 current `tool_needed` rows.
2. The Rabbi audit baseline still tracks 163 contracts: 36 now have local
   wrappers, 12 are registered fallback/setup blockers, and 115 remain
   `tool_wrapper_missing`.
3. Missing-wrapper breakdown: 53 `internal_write`, 30 `read_only`, 28
...
5. Of the 36 wrapper-backed contracts, the first 27 are deployed through
   Railway deployment `2107fae5-1a73-49ec-96e8-5a3a66bb8e43`; the 9
   parent/student summary wrappers are local-verified and still need a clean
   deploy/live smoke path. All 36 still need saved Agent Mode PASS/BLOCKED
   proof before autonomy can be claimed.
6. Natural-language planner coverage is wired for the first alias batch, the
   second read-only batch, and the parent/student summary batch; the remaining
   missing wrappers still need intent routing and safe missing-input prompts.
...
11. Agent Mode has not yet saved a PASS/BLOCKED/FAIL drop-off result for the
   all-163 probe run.
12. Broad `npm test` is currently blocked at 1658/1662 by unrelated Operations
   shell/action-coverage drift (`operations-automation-center`,
   `service-provider-studio-operations-ui`, and two
   `watchdog-action-registry` coverage hash tests). The helper-focused suite
   for this batch is passing 28/28.
