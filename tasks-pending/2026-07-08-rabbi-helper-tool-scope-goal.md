...
| Next requirement IDs to work | Continue `REQ-20260708-095`; 97 contracts are now local wrapper-backed, 66 are deployed/live-smoked, the prior 18 internal-action wrappers are pushed but deploy-blocked after two Railway failures, and the newest 13 content/provider action wrappers are local verified; full autonomy remains blocked by 54 missing wrappers, 12 fallback blockers, deploy/live smoke for 31 local-only wrappers, and saved all-163 Agent Mode proof |
...
| `TASK-20260708-016` | rabbi_helper_wrapper_batches | Implement scoped helper wrapper batches from the generated map. | Codex | `rabbi_sheller_provider` / `one_time_mishnah_class` | `RAW-20260708-028` | `REQ-20260708-095` | Investigate Railway failures for deployment IDs `d970e263-1726-41c1-a694-10c1659503ee` and `75d6f181-e7c6-41a3-9e70-efcc4c61fea1`, then deploy/live-smoke all 31 local-only wrappers and continue the remaining 54 missing wrappers. | internal | Open / partial pushed + deploy blocked + local verified / autonomy blocked |
...
| `REQ-20260708-095` | `src/lib/bna/helper/tool-registry.js`, `planner.js`, `permissions.js`, `safety.js`, parity/scope maps, helper tests, action coverage, lazy-subnav tests | First runtime alias batch plus read-only batch plus parent/student summary batch plus dry-run preview wrappers, draft-only sidekick wrappers, internal-action wrappers, and the content/provider action wrapper batch for agent-result records, One Time video library items, moderated student questions, newsletter revisions, weekly-update hero selection, provider profile updates, and Google Business metadata capture. Scope-map generator treats write-shaped missing wrappers as write/draft/approval-gated instead of read-only. | PASS `node --check src/lib/bna/helper/tool-registry.js`; PASS `node --check src/lib/bna/helper/planner.js`; PASS `node --check scripts/generate-rabbi-helper-tool-scope-map.mjs`; PASS `node scripts/generate-rabbi-helper-tool-scope-map.mjs`; PASS focused helper/scope/Agent Review tests 38/38; refreshed Rabbi scope map 163 contracts with 97 `tool_wrapper_available_local`, 12 fallback blockers, and 54 missing wrappers | `9618a4d4`, `16a69c9e`, `9e611cbd`, `304d68a8`, `d23c9ec2`, `17ff491b`, `b49a74e2`, `381a8f34`, `2895654e`, `7abad605`; current content/provider action batch commit pending | `7abad605`; current content/provider action batch push pending | Railway deployment `eb599c39-36f7-4f80-9d6d-2a8fc5c6406f` `SUCCESS` covers the first 66 wrapper-backed contracts. Deploy attempts for the prior 18 internal-action wrappers failed after image push on `d970e263-1726-41c1-a694-10c1659503ee` and `75d6f181-e7c6-41a3-9e70-efcc4c61fea1`; the newest 13 content/provider action wrappers are local verified and still need deploy/live smoke. |
...
| `REQ-20260708-095` | Open / 97 local wrapper-backed contracts, 66 deployed, 18 deploy-blocked, 13 deploy pending / autonomy blocked | 97 contracts now `tool_wrapper_available_local`; 12 are registered fallback blockers; 54 remain `tool_wrapper_missing`; no read-only wrappers remain missing | `src/lib/bna/helper/tool-registry.js`, `src/lib/bna/helper/planner.js`, `src/lib/bna/helper/permissions.js`, `src/lib/bna/helper/safety.js`, `scripts/generate-rabbi-helper-tool-scope-map.mjs`, action coverage artifacts, scope maps, tests | PASS helper/scope/Agent Review focused tests 38/38 for the current batch; previous deploy/live smoke through Railway deployment `eb599c39-36f7-4f80-9d6d-2a8fc5c6406f` covers the first 66 wrapper-backed contracts | Full autonomy still blocked by 54 missing wrappers, 12 fallback blockers, deploy/live smoke for the 31 local-only wrappers after Railway deployment failure investigation, and saved Agent Mode proof |
...
2. The Rabbi audit baseline still tracks 163 contracts: 97 now have local
   wrappers, 12 are registered fallback/setup blockers, and 54 remain
...
5. The first 66 wrapper-backed contracts are deployed through Railway deployment
   ...
   wrappers are local verified and not deployed yet. All 97 wrapper-backed
...
6. Natural-language planner coverage is wired for the first alias batch, the
   ...
   newest content/provider action batch; the remaining missing wrappers still
   need intent routing and safe missing-input prompts.
...
9. Runtime negative tests now cover the first alias batch, read-only batch,
   ...
   and newest content/provider action batch workspace/project rejection plus
   privacy redaction; parent/student/provider privacy negative tests must
   continue to expand per wrapper batch.
...
11. Agent Mode has not yet saved a PASS/BLOCKED/FAIL drop-off result for the
   all-163 probe run.
12. Latest content/provider action batch verification is focused: `node --check`
   ...
   passed 38/38. Full `npm test`, deploy/live smoke, and saved Agent Mode
   PASS/BLOCKED proof still remain for the newest 13 wrappers; deploy/live
   smoke also remains pending for the prior 18 wrappers because of the Railway
   deployment failures.
...
- Remaining blockers:
  `TELEGRAM_CHAT_ID_RABBI_ELIE_SCHELLER` for live Rabbi Telegram sends, 54
  missing wrappers after the local content/provider action batch, 12
  fallback/setup blockers, deploy/live smoke for the 31 local-only wrappers,
