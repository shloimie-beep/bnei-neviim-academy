...
| TASK-20260709-044 | onetime-live-lag-audit | Capture current OneTime live lag and classify whether it is UI, API, runtime, or cache/static delivery. | Codex | one_time_mishnah_class | RAW-20260709-013 | REQ-20260709-070 | Deploy the static/cache policy fix to OneTime, run live header/readback and lag smoke, then resume visual chrome fixes from REQ-20260709-067. | performance/audit | In progress - first cache/static fix verified locally |
...
| DEC-20260709-012 | Do not treat the lag complaint as only a visual/UI issue. | Direct live readback shows multi-second and variable first-byte responses on HTML, assets, and small APIs; a fresh control tower on 2026-07-10 showed no dirty worktree, no ready packets, and no active UI collision lane. | Codex / next performance lane owner | First ship conservative static/cache delivery policy, then continue deeper runtime/hosting response variability investigation and visual chrome polish separately. | Jump straight to component-level UI tweaks. | Component tweaks may improve polish but will not fix multi-second TTFB or request resets. | Deploy the cache policy fix, live-read headers and lag signals, then decide whether runtime process/hosting inspection or UI chrome is the next batch. | REQ-20260709-070 | Implementation lane open; deploy/live-smoke pending |
...
| REQ-20260709-070 | `https://join.onetimeonetime.com` OneTime public, member, classroom, provider-review, student-review, and parent-review routes | Added a reusable no-write Playwright lag audit, captured direct TTFB readback, validated `PKT-20260709-133`, then implemented the first conservative cache/static delivery fix for public JS/CSS/media while preserving no-store/no-cache for HTML, service worker, manifests, APIs, and Operations shell assets. | PASS `node --check server.js`; PASS `node --test tests/one-time-intake-api-readback.test.js tests/bna-helper-tools.test.js`; PASS `node --check scripts/audit-onetime-live-performance.mjs`; PASS live lag audit captured 18/18 samples needing attention; PASS direct curl readback captured multi-second and variable TTFB on HTML, static assets, and small APIs; PASS `npm run pqc:validate -- ops/prompt-packets/2026-07-09-onetime-live-lag-performance/05-live-lag-performance-fix.product-quality.json` | pending | pending | Deploy/live-smoke pending for the cache policy fix. |
...
| REQ-20260709-070 | In progress - first cache/static fix verified locally | `ops/performance-audits/2026-07-09-onetime-live-lag-audit/report.md` and `ttfb-readback.md` show 18/18 lag samples need attention; direct readback shows slow/variable first-byte responses on HTML, static JS/CSS, and small APIs; `PKT-20260709-133` validates; fresh control tower on 2026-07-10 shows no active UI collision lane. | `raw-input/RAW-20260709-013-onetime-app-lag-ui-followup.md`; `server.js`; `tests/bna-helper-tools.test.js`; `tests/one-time-intake-api-readback.test.js`; `scripts/audit-onetime-live-performance.mjs`; `ops/performance-audits/2026-07-09-onetime-live-lag-audit/*`; `ops/prompt-packets/2026-07-09-onetime-live-lag-performance/05-live-lag-performance-fix.product-quality.json`; this register | PASS `node --check server.js`; PASS `node --test tests/one-time-intake-api-readback.test.js tests/bna-helper-tools.test.js`; PASS live no-write lag audit; PASS direct TTFB/header readback; PASS PQC validation; PASS no external writes | Deploy the static/cache policy fix to OneTime and live-read `Cache-Control` headers before closing this requirement; continue deeper runtime/hosting variability investigation if live TTFB remains slow. |
...
Deployment/live-smoke required before terminal Done:

- Commit and push the cache policy fix.
- Deploy to the OneTime Railway target or confirm approved auto-deploy.
- Live-read headers on `https://join.onetimeonetime.com/js/bna-bot-widget.js`,
  ...
  `/one-time/`, and Operations shell assets.
- Re-run a scoped no-write lag/header readback and update this row with the
  deployed commit/deployment evidence.
