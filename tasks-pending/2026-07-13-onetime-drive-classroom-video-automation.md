...
|---|---|---|---|---|---|---|---|---|---|
| DEC-20260713-006 | Treat the supplied Vimeo values as owner app credential material, not as `VIMEO_ACCESS_TOKEN`, unless a Vimeo `/me` bearer readback succeeds later. | Which supplied value is intended to be a user token versus client ID, and whether a separate valid owner user access token exists. Current evidence: both supplied values returned 401 when tested directly as bearer tokens; the existing local `VIMEO_ACCESS_TOKEN` returned 200 for `/me` and reads account name `Shloimie Dratler` with redacted URI and `account=free`. | Operator + Codex | Keep the supplied owner credential material in the local keyholder, but do not replace the working `VIMEO_ACCESS_TOKEN`; use the existing local access token for user-authenticated account reads until a valid owner bearer token is confirmed/generated. | Replace access token with either supplied value, but that currently breaks `/me`; rerun OAuth flow to create a least-privilege user token. | Misclassifying a client credential as a user token would block account reads and uploads. Keeping the valid access token preserves current read-only readiness. | Choose/create the private Vimeo test project/folder, confirm whether the free account has the needed API upload/private/edit/video_files scopes, and explicitly approve a synthetic private upload smoke before setting `BNA_VIMEO_PRIVATE_SMOKE=1`. | REQ-20260713-918 | Needs operator decision |
...
| REQ-20260713-918 | local keyholder, Vimeo read-only API | Update owner credential material, verify app/client and existing access-token readbacks without upload. | Existing access token `/me` passed and read account name `Shloimie Dratler`; both newly supplied values returned 401 when tested directly as bearer tokens; no-write private smoke returned `preview_only`; read-only owner-confirmed capability check returned `test_target_missing` because no Vimeo private test project/folder is configured. | n/a, secret store outside repo | n/a | not required |
