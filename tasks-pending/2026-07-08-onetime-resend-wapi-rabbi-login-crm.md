...
| REQ-20260708-072 | In progress | Send Shloimie an internal Telegram bot reminder when someone signs up through the OneTime public signup form. | Raw input: `raw-input/RAW-20260708-016-onetime-signup-telegram-reminder.md`. Implementation target: `/api/one-time/interest` and `/api/bna/product-leads` after the lead is saved. Guardrail: Telegram reminder must be non-blocking and must not send parent email, WhatsApp, checkout, payment, access, Zoom, Vimeo, Drive, or CRM external writes. |
...
| B8 | OneTime signup Telegram reminder | `REQ-20260708-072` | In progress |
