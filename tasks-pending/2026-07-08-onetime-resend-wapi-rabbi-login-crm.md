...
| REQ-20260708-049 | Pending / WAPI credential blocked | Prepare a concise OneTime WhatsApp welcome/class-link message and send one approved test to the redacted operator phone from the Rabbi / OneTime sender after WAPI readiness and copy/link are verified. | Current class link is now configured in OneTime Railway runtime, but the actual WhatsApp test remains blocked by missing Rabbi-scoped WAPI sender credentials and approval details; see `DEC-20260708-010` and `DEC-20260708-011`. |
...
| DEC-20260708-011 | WhatsApp test send payload approval. | Final live WhatsApp test still needs verified Rabbi-scoped WAPI sender credentials and exact test recipient/sender approval. The current class link is now configured outside the repo. | Shloimie/Codex | Keep WAPI sends disabled until the Rabbi WAPI token/account is provided and verified, then run a no-send preview/readiness check before sending one test only to the approved recipient. | Send rough ramble copy; do no WhatsApp send. | Sending too early can produce an incorrect live message, expose the wrong sender, or message from a non-Rabbi WhatsApp account. | Provide/verify the Rabbi WAPI credential folder/sender account, then approve the final copy/sender/recipient for one test. | `REQ-20260708-049` | Pending |
