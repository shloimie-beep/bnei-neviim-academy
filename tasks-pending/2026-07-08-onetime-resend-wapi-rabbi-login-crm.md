...
| REQ-20260708-073 | In progress | Make the public OneTime landing helper concise and timed: first nudge after 10 seconds asks whether the visitor wants his son to love Torah; follow-up 20 seconds later says where the class is holding by masechta and that now is a great time to join. | Raw input: `raw-input/RAW-20260708-017-onetime-landing-helper-concise-timed.md`. Implementation target: OneTime public helper only (`public/js/bna-bot-widget.js`, `public/one-time/index.html`, action registry/tests). Guardrails: keep backend/privacy guardrails but remove long public "things I do not do" caveats from the landing helper popup/copy. |
...
| B9 | Concise OneTime landing helper nudges | `REQ-20260708-073` | In progress |
