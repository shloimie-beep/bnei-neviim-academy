...
|---|---|---|---|---|---|---|
| `REQ-20260706-941` | `/api/provider-portal/mailbox`, `/api/provider-portal/mailbox/:threadKey`, `/api/provider-portal/mailbox/:threadKey/draft`, `/api/provider-portal/mailbox/:threadKey/send` | Reuse `bna_communications` Resend inbound/outbound thread model. | PASS `node --check server.js`; PASS `node --test tests/provider-mailbox-portal.test.js`; PASS WAPI regression bundle. | rebased implementation commit `52d8b980`; PR #117 | branch pushed to `origin/codex/onetime-crm-mailbox-20260706` | required; not run |
| `REQ-20260706-942` | `public/provider.html` Mailbox section | Add polished mailbox tab/list/detail/reply composer/readiness card. | PASS inline script parse; PASS action watchdog finding_count=0. | rebased implementation commit `52d8b980`; PR #117 | branch pushed to `origin/codex/onetime-crm-mailbox-20260706` | required; not run |
| `REQ-20260706-943` | `/draft`, `/send`, composer controls | Save local draft rows; send via Resend only after readiness and `SEND_RESEND_EMAIL`. | PASS `tests/provider-mailbox-portal.test.js`; PASS no-send Resend safe smoke `send_allowed=true`; no external send performed. | rebased implementation commit `52d8b980`; PR #117 | branch pushed to `origin/codex/onetime-crm-mailbox-20260706` | required; not run |
| `REQ-20260706-944` | readiness payload | Runtime-only `ONE_TIME_MAILING_ADDRESS` boolean; no tracked address text. | PASS static test; PASS redacted Railway readback for `RESEND_WEBHOOK_SECRET`, sender name, reply-to, and `ONE_TIME_MAILING_ADDRESS`; no physical address committed. | rebased implementation commit `52d8b980`; PR #117 | branch pushed to `origin/codex/onetime-crm-mailbox-20260706` | required; not run |
| `REQ-20260706-945` | route/action registries | Add four API routes and four visible actions. | PASS `npm run watchdog:actions`; PASS JSON parse via tests. | rebased implementation commit `52d8b980`; PR #117 | branch pushed to `origin/codex/onetime-crm-mailbox-20260706` | required; not run |
...
| `REQ-20260706-941` | Draft PR open; deploy/live blocked | Provider mailbox APIs added and static contract passed. | `server.js`; `tests/provider-mailbox-portal.test.js`; PR #117 | `node --check server.js`; `node --test tests\provider-mailbox-portal.test.js` | merge/deploy/live smoke still required; live DB/Resend readback not run |
...
| `REQ-20260706-943` | Local verified; live send not performed | Draft endpoint logs no-send CRM rows; send endpoint requires Resend readiness and exact `SEND_RESEND_EMAIL`, then logs sent row/audit. | `server.js`; `public/provider.html`; `tests/provider-mailbox-portal.test.js`; `ops/one-time-mishnah/integration-smokes/2026-07-06T14-22-03-886Z-resend-vimeo-stripe-safe-smoke.md` | focused mailbox test and WAPI regression bundle PASS; no-send Resend safe smoke PASS with domain check OK and `send_allowed=true` | no external send performed; live send remains approval-gated |
| `REQ-20260706-944` | Runtime configured; deploy/live blocked | Readiness reports webhook, sender, reply-to, and `ONE_TIME_MAILING_ADDRESS` configured/missing as boolean only; Railway runtime now has all four by redacted readback. | `server.js` | focused mailbox test PASS; redacted Railway variable readback PASS | exact address not committed; deploy/live smoke still required |
...
| `REQ-20260706-946` | Release gate local verified; deploy/live blocked | Local mailbox verification passed; draft PR #117 opened; protocol drift blocker fixed; runtime Resend/mailing-address config readback passed. | register, watchdog/PQC reports, safe smoke report, tests, PR #117 | PASS `node --test tests\provider-wapi-setup-portal.test.js tests\provider-mailbox-portal.test.js`; PASS `npm run watchdog:protocol-drift`; PASS `npm run watchdog:actions`; PASS no-send Resend safe smoke at `ops/one-time-mishnah/integration-smokes/2026-07-06T14-22-03-886Z-resend-vimeo-stripe-safe-smoke.md` | merge/deploy/live smoke still required |
