...
| `REQ-20260706-943` | `/draft`, `/send`, composer controls | Save local draft rows; send via Resend only after readiness and `SEND_RESEND_EMAIL`. | PASS `tests/provider-mailbox-portal.test.js`; no external send performed. | implementation commit `7f5246c9`; draft PR #117 | branch pushed to `origin/codex/onetime-crm-mailbox-20260706` | required; not run |
| `REQ-20260706-944` | readiness payload | Runtime-only `ONE_TIME_MAILING_ADDRESS` boolean; no tracked address text. | PASS static test; no physical address committed. | implementation commit `7f5246c9`; draft PR #117 | branch pushed to `origin/codex/onetime-crm-mailbox-20260706` | target env required |
...
| `REQ-20260706-946` | Draft PR open; deploy/live blocked | Local mailbox verification passed; draft PR #117 opened; global protocol drift watchdog failed on unrelated full-UI audit prompt-series artifacts already on current `origin/master`. | register, watchdog/PQC reports, tests, PR #117 | PASS `node --test tests\provider-wapi-setup-portal.test.js tests\provider-mailbox-portal.test.js`; FAIL `npm run watchdog:protocol-drift` due existing `ops/prompt-packets/2026-07-06-onetime-full-ui-agent-audit/*` drift findings, not the mailbox PQC packet | merge/deploy/live smoke still required; global drift blocker belongs to the existing prompt-series packet |
