...
| `REQ-20260709-069` | Send Rabbi the OneTime Telegram bot start instructions by WhatsApp | `Blocked / no send performed` | A single WhatsApp to the saved Rabbi provider contact may be sent only if the recipient, sender, copy, OneTime WAPI sender metadata, and no-secret/no-access-link guardrails are all auditable. The send's purpose is only to ask the intended Rabbi account/group to message `t.me/onetimeaios_bot`, so Codex can read the chat ID candidate safely. | Source `RAW-20260709-012` is contact-private and not committed. 2026-07-09 preflight: PASS `npm run telegram:rabbi:readiness` no-send, still blocked by missing `TELEGRAM_CHAT_ID_RABBI_ELIE_SCHELLER`; PASS `npm run telegram:rabbi:chat-id` no-send, bot resolves as `onetimeaios_bot`, candidate count `0`; EXPECTED BLOCKED `npm run one-time:wapi:readiness`, WAPI token scoped but instance ID/sender phone metadata and auto-reply approval flags missing. No WhatsApp, Telegram, WAPI, CRM, credential, access, or provider mutation was performed. |
...
| `DEC-20260708-021` | Rabbi live Telegram send/readback cannot be completed without the intended chat ID. | Shloimie | Preferred: have the intended Rabbi Telegram account/group message `t.me/onetimeaios_bot`, then let Codex run the safe getUpdates/readiness flow. Alternate: send the approved WhatsApp instruction only after OneTime WAPI sender metadata/approval is auditable; current preflight keeps that WhatsApp send blocked. | `Open` |
