...
| `REQ-20260709-069` | Send Rabbi the OneTime Telegram bot start instructions by WhatsApp | `Sent / local runtime ready / live smoke gated` | One WhatsApp instruction send to the saved Rabbi provider contact is recorded with redacted proof. The Rabbi bot chat candidate has since appeared and local no-send readiness reports `TELEGRAM_CHAT_ID_RABBI_ELIE_SCHELLER` configured. Live Rabbi Telegram delivery remains gated until release gates are otherwise clear and exact live-send smoke scope is approved. | Source `RAW-20260709-012`; redacted send evidence `ops/access/2026-07-09-rabbi-telegram-bot-whatsapp-instructions.md`; communication row `#2909`; provider status `200`; message id fingerprint `6497d51fc20f`. PASS `npm run telegram:rabbi:readiness` no-send at 2026-07-09T17:02:52Z: Rabbi runtime ready, token/chat ID/ops credentials configured. PASS `npm run telegram:rabbi:chat-id` no-send at 2026-07-09T17:02:53Z: bot `onetimeaios_bot`, `candidate_count=4`, `unique_chat_count=1`, masked candidate `******4810`, no token/raw chat ID printed. No Telegram message, credential mutation, access/payment change, DNS change, deploy, or production-data mutation was performed by this reconciliation. |
...
| `DEC-20260708-021` | Rabbi live Telegram send/readback cannot be completed without the intended chat ID. | Shloimie/Rabbi Elie/Codex | WhatsApp instructions were sent to Rabbi Elie on 2026-07-09. A masked Telegram candidate is now available and local no-send readiness reports the Rabbi chat ID configured. Do not run a live Rabbi Telegram send/smoke until release gates are otherwise clear and the exact send scope is approved. | `Local runtime ready / live smoke gated` |
...
- Post-send `npm run telegram:rabbi:chat-id` later found 4 candidates from 1
  ...
  Operations credentials configured. Live Rabbi Telegram delivery/smoke remains
  gated until release gates are otherwise clear and exact live-send scope is
  approved.
