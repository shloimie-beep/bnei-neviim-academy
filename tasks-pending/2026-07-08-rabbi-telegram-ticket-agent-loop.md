...
- Contact-private Rabbi WhatsApp instruction intake:
  `RAW-20260709-012` is local/contact-private and must not be committed with
  raw contact details or private message body.
...
| `REQ-20260709-069` | Send Rabbi the OneTime Telegram bot start instructions by WhatsApp | `Sent / response received / local runtime ready` | One WhatsApp instruction send to the saved Rabbi provider contact is recorded with redacted proof. The Rabbi bot chat candidate has since appeared and local no-send readiness reports `TELEGRAM_CHAT_ID_RABBI_ELIE_SCHELLER` configured. Hosted production still needs a normal restart/deploy window before the running Railway service can rely on the newly set env values. | Source `RAW-20260709-012`; redacted send evidence `ops/access/2026-07-09-rabbi-telegram-bot-whatsapp-instructions.md`; communication row `#2909`; provider status `200`; message id fingerprint `6497d51fc20f`. PASS `npm run telegram:rabbi:readiness` no-send: Rabbi runtime ready, token/chat ID/ops credentials configured. PASS `npm run telegram:rabbi:chat-id` no-send: bot `onetimeaios_bot`, `candidate_count=4`, `unique_chat_count=1`, masked candidate `******4810`, no token/raw chat ID printed. Ignored local runtime config and OneTime Railway variable keys were set with values hidden and `--skip-deploys`; no deploy was triggered. Local bridge restart/status passed after the Kimi thinking-mode fix. No raw token/chat ID/phone was committed. |
...
| --- | --- | --- | --- | --- |
| `DEC-20260708-016` | Rabbi Telegram chat ID is required before the Rabbi worker can run live. | Shloimie/Rabbi Elie/Codex | Rabbi messaged the bot on 2026-07-09. Codex inferred the single verified private chat, configured the local/Railway chat ID aliases with values hidden, and restarted the local bridge. Hosted production still needs the normal restart/deploy window before relying on those Railway env values. | `Resolved locally / hosted restart pending` |
...
| `DEC-20260708-021` | Rabbi live Telegram send/readback cannot be completed without the intended chat ID. | Shloimie/Rabbi Elie/Codex | WhatsApp instructions were sent to Rabbi Elie on 2026-07-09. Rabbi messaged `t.me/onetimeaios_bot`; Codex selected the single verified private chat masked `******4810`, configured local and Railway chat ID aliases with values hidden, and restarted the local bridge. Hosted production still needs restart/deploy and normal live-smoke approval. | `Resolved locally / hosted restart pending` |
...
- 2026-07-09 WhatsApp instruction send: Shloimie explicitly approved a real
  WhatsApp to Rabbi Scheller with the Telegram bot instructions. Codex sent one
  WhatsApp from the OneTime/Rabbi WAPI sender to the saved Rabbi provider
  contact ending `3006`. Communication row `#2909`; provider status `200`;
  message id fingerprint `6497d51fc20f`. Evidence:
  `ops/access/2026-07-09-rabbi-telegram-bot-whatsapp-instructions.md`.
- Post-send `npm run telegram:rabbi:chat-id` later found 4 candidates from 1
  unique masked private chat `******4810`, and no-send
  `npm run telegram:rabbi:readiness` reports Rabbi token, chat ID, and OneTime
  Operations credentials configured. Codex configured ignored local runtime
  chat ID aliases and OneTime Railway production variable keys with values
  hidden and `--skip-deploys`.
- The local Rabbi bridge was restarted and is running after the Kimi
  thinking-mode compatibility fix. Hosted production still needs a normal
  restart/deploy window before the running Railway service can rely on the
  newly set env values.
