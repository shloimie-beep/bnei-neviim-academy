...
|---|---|---|---|---|---|---|---|---|
| TASK-20260709-031 | onetime-free-zoom-lead-capture | Finish immediate OneTime lead capture path for free-class follow-up. | Codex | one_time_mishnah_class | RAW-20260709-008 | REQ-20260709-034, REQ-20260709-035 | Commit, push, deploy, live-smoke. | launch-critical | Implemented / local verified |
...
|---|---|---|---|---|---|---|
| REQ-20260709-032 | `ops/execution-runs/.../STATUS.md`, `NEXT-SESSION.md` | Replace stale Railway target blocker with current target guard readback and leave full setup blockers separate. | `npm run one-time:railway-target:guard`; `npm run one-time:setup:check` readback | Pending | Pending | Not required for docs |
| REQ-20260709-034 | `/api/one-time/interest`, `bna_product_leads`, `bna_parent_leads`, `bna_contact_communications` | Insert product lead, upsert CRM lead, log internal note, expose CRM linkage in safe response. | PASS `node --check server.js`; PASS focused tests; PASS Playwright intercepted form POST | Pending | Pending | Required |
| REQ-20260709-035 | `/one-time` public landing | Add practical lead fields and free-class/Zoom copy; remove immediate portal promise. | PASS focused tests; PASS local Playwright 1440/390 smoke with 0px overflow | Pending | Pending | Required |
| REQ-20260709-036 | OneTime helper/bot | Scoped helper now points to the free-class form. It is a guided handoff, not direct in-chat lead submission. | PASS `tests/one-time-brand-helper-isolation.test.js`; PASS `npm run watchdog:actions` | Pending | Pending | Required if changed |

## Final audit

| ID | Status | Evidence | Files changed | Verification | Remaining issue |
|---|---|---|---|---|---|
| REQ-20260709-031 | Done except final closeout append | Raw/register/memory created; durable memory added to `MEMORY.md`. | raw-input/RAW-20260709-008-onetime-lead-capture-free-zoom-ui-priority.md; this file; memory/2026-07-09.md; MEMORY.md | Pending ledger/changelog closeout | None |
...
| REQ-20260709-034 | Implemented / local verified | `/api/one-time/interest` now writes product lead, upserts `bna_parent_leads`, logs `bna_contact_communications`, returns `internal_crm_recorded`, `crm_lead_id`, and no-send/no-checkout/no-access flags. | server.js; tests/one-time-product-system.test.js | PASS `node --check server.js`; PASS focused tests | Needs commit/push/deploy/live smoke. |
| REQ-20260709-035 | Implemented / local verified | Landing form now captures parent name/email/student/phone; copy says free-class/Zoom follow-up and no portal account. Local Playwright intercepted POST with expected keys and 0px overflow at 1440x900 and 390x844. | public/one-time/index.html; tests/one-time-focused-landing.test.js | PASS focused tests; PASS local Playwright smoke with intercepted POST | Needs deploy/live smoke. |
...
| REQ-20260709-040 | Pending release | Local checks pass: focused tests, action watchdog, protocol drift watchdog, secrets audit, execution-run validator. | git/release | Pending commit/push/deploy/live smoke | Release still in progress. |
