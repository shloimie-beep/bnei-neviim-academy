...
| REQ-20260709-034 | Make public `/api/one-time/interest` create/update first-party CRM lead records, not only product lead rows. | RAW-20260709-008 | one_time_mishnah_class | Codex | backend-crm | P0 | 2 | REQ-20260709-033 | Interest submit writes `bna_product_leads`, upserts `bna_parent_leads`, logs internal communication/follow-up evidence, returns no-send/no-checkout/no-access flags, and does not perform external writes. | server.js, tests | yes | Done / deployed; dry-run proof locally verified pending release |
...
| REQ-20260709-034 | Done / deployed; dry-run proof locally verified | `/api/one-time/interest` now writes product lead, upserts `bna_parent_leads`, logs `bna_contact_communications`, returns `internal_crm_recorded`, `crm_lead_id`, and no-send/no-checkout/no-access flags. Local Playwright intercepted POST proved the submitted fields without mutating production. New `dry_run=true` path validates the same payload shape and returns product/CRM/internal-note previews without creating a product lead, CRM lead, internal note, Telegram reminder, checkout, access grant, Zoom meeting, or external write. | server.js; tests/one-time-product-system.test.js; scripts/smoke-one-time-interest-dry-run-live.mjs; package.json | PASS `node --check server.js`; PASS `node --check scripts\smoke-one-time-interest-dry-run-live.mjs`; PASS focused tests; PASS `npm test` 1697/1697; PASS local intercepted POST; PASS live target/read-only smokes | Dry-run proof still needs commit, push, OneTime deploy, and live smoke against `https://join.onetimeonetime.com`. Real-write production POST remains intentionally unrun because it would create a real CRM lead and trigger the internal Telegram reminder. |
...
| REQ-20260709-040 | Prior launch done / deployed / live-smoked; dry-run proof pending release | Commit `2931b1cc` pushed to `origin/codex/rabbi-helper-tool-scope-20260708`; explicit OneTime Railway CLI deploy reached `SUCCESS` for deployment `02db803b-8c6b-45fd-89d8-54af8f12f6c9`; live smokes passed. Current dry-run proof batch is locally verified and awaiting scoped commit/push/deploy/live smoke. | git/release; scripts/smoke-onetime-separate-instance-live.mjs; scripts/smoke-one-time-interest-dry-run-live.mjs | PASS `npm run one-time:target:guard -- --json` with only local Railway-link warning; PASS `npm run one-time:railway-target:guard`; PASS focused tests; PASS `npm test` 1697/1697; previous live smokes passed | Complete the dry-run proof release, then record the new deployment ID and live smoke report. |
