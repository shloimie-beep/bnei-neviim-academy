...
| REQ-20260709-034 | `/api/one-time/interest`, `bna_product_leads`, `bna_parent_leads`, `bna_contact_communications` | Insert product lead, upsert CRM lead, log internal note, expose CRM linkage in safe response. Add `dry_run=true` preview path so production can be live-smoked without creating a lead or sending the Telegram reminder. | PASS `node --check server.js`; PASS focused tests; PASS `npm test` 1697/1697; PASS Playwright intercepted form POST; PASS live dry-run POST smoke | 2931b1cc; 45b332b6 | 2931b1cc; 45b332b6 | Prior Railway `02db803b-8c6b-45fd-89d8-54af8f12f6c9` SUCCESS; dry-run proof Railway `0c1eec63-aa58-4a65-8bc0-0262ba626401` SUCCESS; PASS `ops/live-smokes/2026-07-09T14-10-19-816Z-one-time-interest-dry-run-live-smoke.md` |
...
| REQ-20260709-034 | Done / deployed / live-smoked | `/api/one-time/interest` now writes product lead, upserts `bna_parent_leads`, logs `bna_contact_communications`, returns `internal_crm_recorded`, `crm_lead_id`, and no-send/no-checkout/no-access flags. Local Playwright intercepted POST proved the submitted fields without mutating production. New `dry_run=true` path validates the same payload shape and returns product/CRM/internal-note previews without creating a product lead, CRM lead, internal note, Telegram reminder, checkout, access grant, Zoom meeting, or external write. | server.js; tests/one-time-product-system.test.js; scripts/smoke-one-time-interest-dry-run-live.mjs; package.json; ops/live-smokes/2026-07-09T14-10-19-816Z-one-time-interest-dry-run-live-smoke.md | PASS `node --check server.js`; PASS `node --check scripts\smoke-one-time-interest-dry-run-live.mjs`; PASS focused tests; PASS `npm test` 1697/1697; PASS local intercepted POST; PASS live target/read-only smokes; PASS live dry-run POST smoke | Real-write production POST remains intentionally unrun because it would create a real CRM lead and trigger the internal Telegram reminder. Dry-run proof covers the non-mutating production POST mapping. |
...
| REQ-20260709-040 | Done / deployed / live-smoked | Commit `2931b1cc` pushed to `origin/codex/rabbi-helper-tool-scope-20260708`; commit `45b332b6` pushed to `origin/master`; explicit OneTime Railway CLI deploy reached `SUCCESS` for deployment `0c1eec63-aa58-4a65-8bc0-0262ba626401`; live smokes passed. | git/release; scripts/smoke-onetime-separate-instance-live.mjs; scripts/smoke-rabbi-onetime-landing-live.mjs; scripts/smoke-one-time-interest-dry-run-live.mjs; ops/live-smokes/2026-07-09T14-10-19-816Z-one-time-interest-dry-run-live-smoke.md | PASS `npm run one-time:target:guard -- --json`; PASS `npm run one-time:railway-target:guard`; PASS deployment status poll; PASS `npm run app:smoke:onetime-separate-instance -- https://join.onetimeonetime.com`; PASS `npm run app:smoke:rabbi-onetime-landing -- https://join.onetimeonetime.com`; PASS `npm run app:smoke:one-time-interest-dry-run` | Full launch setup still blocked by payment/WAPI/campaign approvals; lead-capture dry-run proof is closed. |
