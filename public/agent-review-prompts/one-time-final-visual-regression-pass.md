# Agent Mode Prompt - One Time Final Visual Regression Pass
...
```json
{
  ...
  "prompt_key": "one-time-final-visual-regression-pass",
  "context_key": null,
  "public_prompt_url": "https://join.onetimeonetime.com/agent-review-prompts/one-time-final-visual-regression-pass.md",
  "public_artifacts": [],
  "return_url": "https://bneineviimacademy.org/operations/agent-review?prompt=one-time-final-visual-regression-pass",
  ...
  "idempotency_key": "2026-06-26-agent-review-dropoff-repair:one-time-final-visual-regression-pass:all-contexts"
}
```

## Start

Open this public prompt first: https://join.onetimeonetime.com/agent-review-prompts/one-time-final-visual-regression-pass.md
...
Public prompt URL: https://join.onetimeonetime.com/agent-review-prompts/one-time-final-visual-regression-pass.md
...
1. Run this only after at least one implementation packet has been applied, deployed if app-visible, and live-smoked. If no implementation has been applied, save BLOCKED with "implementation not yet applied".
2. Open /operations/agent-review?prompt=one-time-final-visual-regression-pass first when available, then public prompt and drop-off.
3. Open /one-time, /one-time/mishnayos, /rabbi-member, /member-library, /one-time-classroom, /one-time-classroom.html?review=one-time&code=TEST-ONETIME-REVIEW-ACCESS, /provider.html?review=one-time, and authenticated scoped Operations routes if available and safe to redact.
4. Capture 1440px, 1024px, 768px, 430px, and 390px first viewport screenshots plus targeted header/footer/helper/overflow crops on any route that changed.
5. Verify all prior P0/P1 findings from the current audit package are resolved or explicitly still blocked with owner and next action.
6. Verify live route readback and deployment evidence when app-visible files changed. Do not mark PASS from local-only screenshots for app-visible UI changes.
7. Do not perform external sends, payments, access grants, provider writes, or credential actions while verifying.
8. Save PASS, FAIL, or BLOCKED through Agent Review drop-off. Do not ask the operator whether to submit/seal the final result.
