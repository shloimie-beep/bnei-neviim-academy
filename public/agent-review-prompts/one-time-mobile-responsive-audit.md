# Agent Mode Prompt - One Time Mobile Responsive Audit
...
```json
{
  ...
  "prompt_key": "one-time-mobile-responsive-audit",
  "context_key": null,
  "public_prompt_url": "https://join.onetimeonetime.com/agent-review-prompts/one-time-mobile-responsive-audit.md",
  "public_artifacts": [],
  "return_url": "https://bneineviimacademy.org/operations/agent-review?prompt=one-time-mobile-responsive-audit",
  ...
  "idempotency_key": "2026-06-26-agent-review-dropoff-repair:one-time-mobile-responsive-audit:all-contexts"
}
```

## Start

Open this public prompt first: https://join.onetimeonetime.com/agent-review-prompts/one-time-mobile-responsive-audit.md
...
Public prompt URL: https://join.onetimeonetime.com/agent-review-prompts/one-time-mobile-responsive-audit.md
...
1. Open /operations/agent-review?prompt=one-time-mobile-responsive-audit first when available, then public prompt and drop-off.
2. Set viewport to 430px and 390px for every route: /one-time, /one-time/mishnayos, /rabbi-member, /member-library, /one-time-classroom, /one-time-classroom.html?review=one-time&code=TEST-ONETIME-REVIEW-ACCESS, and /provider.html?review=one-time.
3. For each route capture full-page, first-viewport, header/topbar crop, footer crop, and any overflow/clipped-control crop.
4. Measure or visually verify page horizontal overflow, text clipping, vertical-letter nav, helper overlap, tap targets under 44px, first useful content y-position, and whether sticky elements obscure content.
5. Open mobile menus/drawers only where safe and record close/back behavior. Do not submit forms with real data or mutate state.
6. For Operations routes, test only if authenticated and redaction is possible; otherwise record auth/privacy blocker and continue public/review routes.
7. Save PASS, FAIL, or BLOCKED through Agent Review drop-off when evidence is enough.
