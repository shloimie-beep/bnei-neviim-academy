# Agent Mode Prompt - One Time Brand Asset Proof Strip Audit
...
```json
{
  ...
  "prompt_key": "one-time-brand-asset-proof-strip-audit",
  "context_key": null,
  "public_prompt_url": "https://join.onetimeonetime.com/agent-review-prompts/one-time-brand-asset-proof-strip-audit.md",
  "public_artifacts": [],
  "return_url": "https://bneineviimacademy.org/operations/agent-review?prompt=one-time-brand-asset-proof-strip-audit",
  ...
  "idempotency_key": "2026-06-26-agent-review-dropoff-repair:one-time-brand-asset-proof-strip-audit:all-contexts"
}
```

## Start

Open this public prompt first: https://join.onetimeonetime.com/agent-review-prompts/one-time-brand-asset-proof-strip-audit.md
...
Public prompt URL: https://join.onetimeonetime.com/agent-review-prompts/one-time-brand-asset-proof-strip-audit.md
...
1. Open /operations/agent-review?prompt=one-time-brand-asset-proof-strip-audit first when available, then public prompt and drop-off.
2. Open /one-time and /one-time/mishnayos at 1440px, 1024px, 768px, 430px, and 390px.
3. Capture full-page, first-viewport, header/logo crop, hero/media crop, proof-strip crop, and footer crop.
4. Inventory visible assets against config/service-provider-sites/one-time.json: logo, hero portrait, promo stage stills, TorahAnytime, 24Six, Loop, and Mishpacha proof logos.
5. If Naki or another proof logo is missing as a real asset, mark asset-needed or omission. Do not accept plain text as a fake logo.
6. Inspect whether images are clear enough to sell the real product rather than dark blurred stock-style decoration.
7. Check colors: One Time black/yellow/cream only, no BNA navy/teal/cream bleed except where explicitly part of shared platform chrome outside the One Time surface.
8. Save PASS, FAIL, or BLOCKED through Agent Review drop-off with an asset table and screenshot paths.
